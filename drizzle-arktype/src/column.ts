...
export function columnToSchema(column: Column): Type {
	...
	if (!schema) {
		...
		if (isColumnType<PgGeometry<any> | PgPointTuple<any>>(column, ['PgGeometry', 'PgPointTuple'])) {
			...
			// DateDuration, LocalTime, LocalDate, LocalDateTime). drizzle-arktype has no
			...
			// `type.unknown` catch-all, which validated nothing (#6027).
			schema = type.unknown.narrow((value) => typeof value === 'object' && value !== null).describe(
				'a Gel temporal value',
			);
		}
	}
