...
export function columnToSchema(column: Column): v.GenericSchema {
	...
	if (!schema) {
		...
		if (isColumnType<PgGeometry<any> | PgPointTuple<any>>(column, ['PgGeometry', 'PgPointTuple'])) {
			...
			// DateDuration, LocalTime, LocalDate, LocalDateTime). drizzle-valibot has no
			...
			// `v.any()` catch-all, which validated nothing (#6027).
			schema = v.custom<object>((value) => typeof value === 'object' && value !== null);
		}
	}
