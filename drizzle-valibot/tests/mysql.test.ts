...
test('all data types', (t) => {
	...
	const expected = v.object({
		...
		bigint3: v.string(),
		bigint4: v.pipe(v.number(), v.minValue(0 as number), v.maxValue(Number.MAX_SAFE_INTEGER), v.integer()),
		bigint5: v.pipe(v.bigint(), v.minValue(0n as bigint), v.maxValue(CONSTANTS.INT64_UNSIGNED_MAX)),
		bigint6: v.string(),
		binary: v.string(),
		boolean: v.boolean(),
