...
> {
	...
	public execute = async (): Promise<TRes> => {
		const queryBuilder = Select.from(this._table, this._partial)
	...
	protected fullOrPartial<Table extends AbstractTable<Table>, Partial extends PartialFor<Table>>(
		mappedServiceToDb: {
		...
		partial?: Partial,
		joinId?: number,
	): Array<FullOrPartial<Table, Partial>> {
		return QueryResponseMapper.map(partial ?? mappedServiceToDb, result, joinId);
	}
	...
	protected abstract joins(): Array<{
		join: Join<any>;
		partial?: { [name: string]: AbstractColumn<ColumnType<any>, boolean, boolean, any> };
		id?: number;
	}>;
}
