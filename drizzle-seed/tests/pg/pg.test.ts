...
import { createSelectSchema } from 'drizzle-orm/validations/zod';
...
beforeAll(async () => {
	...
	await db.execute(
		sql`
			create table "seeder_lib_pg"."test_sequences"
			(
			    col1 integer generated always as identity,
			    col2 bigint generated by default as identity,
				col3 bigint generated by default as identity,
			    col4 smallint,
			    col5 serial,
			    col6 bigserial,
				col7 bigserial,
...
test('seeding table with sequences', async () => {
	await seed(db, { testSequences: schema.testSequences }, { count: 10 }).refine((funcs) => ({
		testSequences: {
			columns: {
				col1: funcs.intPrimaryKey(),
				col2: funcs.intPrimaryKey(),
				col3: funcs.intPrimaryKey(),
				col4: funcs.intPrimaryKey(),
				col5: funcs.intPrimaryKey(),
				col6: funcs.intPrimaryKey(),
				col7: funcs.intPrimaryKey(),
				col8: funcs.intPrimaryKey(),
			},
		},
	}));

	await db.insert(schema.testSequences).values([{}]);
	const res = await db.select().from(schema.testSequences);
