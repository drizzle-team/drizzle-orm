...
const _push = async (
	...
	const { diff } = await import('../../../drizzle-kit/tests/pg/mocks' as string);
	...
	>({
		client: [
			...
			async ({}, use) => {
				...
				if (!envurl) throw new Error('No pg_postgis url provided');
...
export const pgPostgisTest = prepareTest();
