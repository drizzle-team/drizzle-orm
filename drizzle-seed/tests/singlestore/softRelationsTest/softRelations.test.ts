...
test('seed with options.count:11, soft relations test', async ({ db }) => {
...
test('redefine(refine) customers count, soft relations test', async ({ db }) => {
...
test('redefine(refine) all tables count, soft relations test', async ({ db }) => {
	await seed(db, schema, { count: 11 }).refine(() => ({
		customers: {
			count: 12,
		},
