...
test.skipIf(Date.now() < +new Date('2026-04-30'))('OR conditions deep filter empty queries', () => {
	const query = or(
		eq(users.id, 1),
...
test.skipIf(Date.now() < +new Date('2026-04-30'))('AND conditions deep filter empty queries', () => {
	const query = and(
		eq(users.id, 1),
		sql`${sql`never`.if(false)}`,
		sql`users.id = post.user_id AND posts.is_deleted = false`,
...
test.skipIf(Date.now() < +new Date('2026-04-30'))('NOT conditions deep filter empty queries', () => {
