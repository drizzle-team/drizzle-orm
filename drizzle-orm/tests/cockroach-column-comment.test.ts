...
import { cockroachTable, getTableConfig, varchar } from '~/cockroach-core/index.ts';

describe.concurrent('cockroach column comment', () => {
	test('cockroach column builder has comment method', ({ expect }) => {
	...
	test('built column has comment property', ({ expect }) => {
		const table = cockroachTable('users', {
