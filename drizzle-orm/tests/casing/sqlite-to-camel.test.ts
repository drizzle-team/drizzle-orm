...
describe('sqlite to camel case', () => {
  ...
  it('select', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "users"."firstName" || \' \' || "users"."lastName" as "name", "users"."AGE" from "users" left join "developers" on "users"."id" = "developers"."userId" order by "users"."firstName" asc',
      params: [],
    });
  ...
  it('query (find first)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "id", "AGE", "firstName" || \' \' || "lastName" as "name", (select json_array("usesDrizzleOrm") as "data" from (select * from "developers" "users_developers" where "users_developers"."userId" = "users"."id" limit ?) "users_developers") as "developers" from "users" where "users"."id" = ? limit ?',
      params: [1, 1, 1],
  ...
  it('query (find many)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "id", "AGE", "firstName" || \' \' || "lastName" as "name", (select json_array("usesDrizzleOrm") as "data" from (select * from "developers" "users_developers" where "users_developers"."userId" = "users"."id" limit ?) "users_developers") as "developers" from "users" where "users"."id" = ?',
      params: [1, 1],
      typings: ['none', 'none']
    });
    expect(db.dialect.casing.cache).toEqual(cache);
  ...
  it('insert (on conflict do nothing)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'insert into "users" ("id", "firstName", "lastName", "AGE") values (null, ?, ?, ?) on conflict ("users"."firstName") do nothing returning "firstName", "AGE"',
      params: ['John', 'Doe', 30],
    });
    expect(db.dialect.casing.cache).toEqual(usersCache);
  });
  ...
  it('insert (on conflict do update)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'insert into "users" ("id", "firstName", "lastName", "AGE") values (null, ?, ?, ?) on conflict ("users"."firstName") do update set "AGE" = ? returning "firstName", "AGE"',
  ...
  it('update', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'update "users" set "firstName" = ?, "lastName" = ?, "AGE" = ? where "users"."id" = ? returning "firstName", "AGE"',
