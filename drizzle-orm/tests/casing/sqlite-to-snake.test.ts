...
describe('sqlite to camel case', () => {
  ...
  it('select', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "users"."first_name" || \' \' || "users"."last_name" as "name", "users"."AGE" from "users" left join "developers" on "users"."id" = "developers"."user_id" order by "users"."first_name" asc',
      params: [],
    });
  ...
  it('query (find first)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "id", "AGE", "first_name" || \' \' || "last_name" as "name", (select json_array("uses_drizzle_orm") as "data" from (select * from "developers" "users_developers" where "users_developers"."user_id" = "users"."id" limit ?) "users_developers") as "developers" from "users" where "users"."id" = ? limit ?',
      params: [1, 1, 1],
  ...
  it('query (find many)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'select "id", "AGE", "first_name" || \' \' || "last_name" as "name", (select json_array("uses_drizzle_orm") as "data" from (select * from "developers" "users_developers" where "users_developers"."user_id" = "users"."id" limit ?) "users_developers") as "developers" from "users" where "users"."id" = ?',
      params: [1, 1],
      typings: ['none', 'none']
    });
    expect(db.dialect.casing.cache).toEqual(cache);
  ...
  it('insert (on conflict do nothing)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'insert into "users" ("id", "first_name", "last_name", "AGE") values (null, ?, ?, ?) on conflict ("users"."first_name") do nothing returning "first_name", "AGE"',
      params: ['John', 'Doe', 30],
    });
    expect(db.dialect.casing.cache).toEqual(usersCache);
  });
  ...
  it('insert (on conflict do update)', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'insert into "users" ("id", "first_name", "last_name", "AGE") values (null, ?, ?, ?) on conflict ("users"."first_name") do update set "AGE" = ? returning "first_name", "AGE"',
  ...
  it('update', ({ expect }) => {
    ...
    expect(query.toSQL()).toEqual({
      sql: 'update "users" set "first_name" = ?, "last_name" = ?, "AGE" = ? where "users"."id" = ? returning "first_name", "AGE"',
