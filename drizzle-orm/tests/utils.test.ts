...
test('keeps nested object when first column is null but subsequent field is a non-null SQL expression', () => {
	const columns = orderSelectedFields({
		branding: {
			logo: orgBrandingTable.logo,
			active: sql<string>`'active'`,
		},
	});

	const result = mapResultRow(columns, [null, 'active'], { org_branding: false });

	expect(result).toEqual({
		branding: { logo: null, active: 'active' },
	});
});

test('keeps nested object with columns from multiple tables when one is non-null', () => {
	const columns = orderSelectedFields({
		branding: {
			logo: orgBrandingTable.logo,
			theme: orgThemeTable.themeName,
		},
	});
