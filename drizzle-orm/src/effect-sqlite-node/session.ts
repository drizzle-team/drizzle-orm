...
export interface EffectSQLiteNodeQueryEffectHKT extends QueryEffectHKTBase {
	...
	extends SQLiteEffectSession<EffectSQLiteNodeRunResult, EffectSQLiteNodeQueryEffectHKT, TRelations>
{
	...
	constructor(
		...
		private options: EffectSQLiteNodeSessionOptions,
	) {
		super(dialect);
	}
	...
	override prepareQuery<T extends PreparedQueryConfig = PreparedQueryConfig>(
	...
	): SQLiteEffectPreparedQuery<T, EffectSQLiteNodeQueryEffectHKT> {
		...
		return new SQLiteEffectPreparedQuery<T, EffectSQLiteNodeQueryEffectHKT>(
			executeMethod,
			executors,
			query,
			mapper,
...
export class EffectSQLiteNodeTransaction<TRelations extends AnyRelations>
	extends SQLiteEffectTransaction<EffectSQLiteNodeQueryEffectHKT, EffectSQLiteNodeRunResult, TRelations>
{
	static override readonly [entityKind]: string = 'EffectSQLiteNodeTransaction';

	override transaction<A, E, R>(
		transaction: (
			tx: SQLiteEffectTransaction<EffectSQLiteNodeQueryEffectHKT, EffectSQLiteNodeRunResult, TRelations>,
		) => Effect.Effect<A, E, R>,
	): Effect.Effect<A, E | SqlError, R> {
