...
import { drizzle as drizzleNodePg, NodePgDatabase } from '../node-postgres/driver.ts';
...
export class NetlifyDbDriver {
	...
	): NetlifyDbSession<Record<string, unknown>, TablesRelationalConfig> {
...
> extends PgDatabase<NeonHttpQueryResultHKT, TSchema> {
	...
	/** @internal */
	declare readonly session: NetlifyDbSession<TSchema, ExtractTablesWithRelations<TSchema>>;
...
export function drizzle<
	TSchema extends Record<string, unknown> = Record<string, never>,
>(): (NetlifyDbDatabase<TSchema> & { $client: NetlifyDbClient }) | (NodePgDatabase<TSchema> & { $client: NodePgClient });
...
>(
	...
	if (params.length === 0 || (params.length === 1 && isConfig(params[0]) && !('connection' in (params[0] as any)) && !('client' in (params[0] as any)))) {
