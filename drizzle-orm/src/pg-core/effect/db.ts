...
import { type SQL, sql, type SQLWrapper } from '~/sql/sql.ts';
import type { WithSubquery } from '~/subquery.ts';
...
> {
	...
	wrapped: PgDatabase<TQueryResult, TFullSchema, TRelations, TSchema>;
	$with: WithBuilder;
	...
	constructor(
		...
		this.wrapped = new PgDatabase<TQueryResult, TFullSchema, TRelations, TSchema>(
			dialect,
			session,
			relations,
			schema,
	...
	$count(
		...
		return this.wrapped.$count(source, filters);
	}
