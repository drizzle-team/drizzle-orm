...
import { BaseFirebirdDatabase } from './db.ts';
import type { FirebirdRaw } from './query-builders/raw.ts';
...
export abstract class FirebirdPreparedQuery<T extends PreparedQueryConfig> implements PreparedQuery {
	...
	constructor(
		...
		private executeMethod: FirebirdExecuteMethod,
...
export interface FirebirdTransactionConfig {
	behavior?: 'deferred' | 'immediate' | 'exclusive';
}

export type FirebirdExecuteMethod = 'run' | 'all' | 'get';

export abstract class FirebirdSession<
	TResultKind extends 'sync' | 'async',
	TRunResult,
	TFullSchema extends Record<string, unknown>,
	TSchema extends TablesRelationalConfig,
> {
	static readonly [entityKind]: string = 'FirebirdSession';
	...
	): FirebirdPreparedQuery<PreparedQueryConfig & { type: TResultKind }>;
...
export type DBResult<TKind extends 'sync' | 'async', TResult> = { sync: TResult; async: FirebirdRaw<TResult> }[TKind];

export abstract class FirebirdTransaction<
	TResultType extends 'sync' | 'async',
	TRunResult,
	TFullSchema extends Record<string, unknown>,
	TSchema extends TablesRelationalConfig,
> extends BaseFirebirdDatabase<TResultType, TRunResult, TFullSchema, TSchema> {
	static override readonly [entityKind]: string = 'FirebirdTransaction';

	constructor(
		resultType: TResultType,
		dialect: { sync: FirebirdSyncDialect; async: FirebirdAsyncDialect }[TResultType],
		session: FirebirdSession<TResultType, TRunResult, TFullSchema, TSchema>,
		protected schema: {
