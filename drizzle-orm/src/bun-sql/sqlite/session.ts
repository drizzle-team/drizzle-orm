...
> extends SQLiteSession<'async', void, TFullSchema, TSchema> {
	...
	// override async get<T = unknown>(query: DrizzleSQL): Result<'async', T> {
	// 	console.trace('get override', query);

	// 	const res = await this.prepareOneTimeQuery(this.dialect.sqlToQuery(query), undefined, 'run', false).get() as Result<
	// 		'async',
	// 		T
	// 	>;
	// 	console.trace(res);
