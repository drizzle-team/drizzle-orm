	...
	extends MySqlDatabase<PrismaMySqlQueryResultHKT, PrismaMySqlPreparedQueryHKT, Record<string, never>>
{
	...
	constructor(client: PrismaClient, logger: Logger | undefined, extensions?: DrizzleMySqlExtension[]) {
		const dialect = new MySqlDialect();
		super(dialect, new PrismaMySqlSession(dialect, client, { logger }), undefined, 'default', extensions);
	}
}

export type PrismaMySqlConfig = Omit<DrizzleConfig<Record<string, unknown>, DrizzleMySqlExtension>, 'schema'>;
...
export function drizzle(config: PrismaMySqlConfig = {}) {
	...
	return Prisma.defineExtension((client) => {
		return client.$extends({
			name: 'drizzle',
			client: {
				$drizzle: new PrismaMySqlDatabase(client, logger, config.extensions),
			},
		});
	});
}
