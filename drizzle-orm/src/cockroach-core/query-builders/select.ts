...
import type { SubqueryWithSelection } from '~/cockroach-core/subquery.ts';
import type { CockroachTable } from '~/cockroach-core/table.ts';
import { CockroachViewBase } from '~/cockroach-core/view-base.ts';
...
import type {
	...
	CockroachSelectJoinFn,
	CockroachSelectPrepare,
	CockroachSelectWithout,
	CockroachSetOperatorExcludedMethods,
	CockroachSetOperatorWithResult,
	CreateCockroachSelectFromBuilderMode,
	GetCockroachSetOperators,
	LockConfig,
	LockStrength,
	SelectedFields,
...
> {
	...
	private session: CockroachSession | undefined;
	...
	> {
		...
		} else if (is(src, CockroachViewBase)) {
			fields = src[ViewBaseConfig].selectedFields as SelectedFields;
		} else if (is(src, SQL)) {
			fields = {};
		} else {
			fields = getTableColumns<CockroachTable>(src);
		}
...
export abstract class CockroachSelectQueryBuilderBase<
	THKT extends CockroachSelectHKTBase,
...
> extends TypedQueryBuilder<TSelectedFields, TResult> {
	static override readonly [entityKind]: string = 'CockroachSelectQueryBuilder';
	...
	private createSetOperator(
		...
		rightSelection:
			| ((setOperators: GetCockroachSetOperators) => SetOperatorRightSelect<TValue, TResult>)
	...
	> {
		return (rightSelection) => {
			const rightSelect = (typeof rightSelection === 'function'
				? rightSelection(getCockroachSetOperators())
	...
	/** @internal */
	addSetOperators(setOperators: CockroachSelectConfig['setOperators']): CockroachSelectWithout<
		this,
		TDynamic,
		CockroachSetOperatorExcludedMethods,
		true
	...
	): CockroachSelectWithout<this, TDynamic, 'where'> {
	...
	): CockroachSelectWithout<this, TDynamic, 'having'> {
	 ...
	 */
	groupBy(
		builder: (aliases: this['_']['selection']) => ValueOrArray<CockroachColumn | SQL | SQL.Aliased>,
	): CockroachSelectWithout<this, TDynamic, 'groupBy'>;
	groupBy(...columns: (CockroachColumn | SQL | SQL.Aliased)[]): CockroachSelectWithout<this, TDynamic, 'groupBy'>;
	groupBy(
		...columns:
			| [(aliases: this['_']['selection']) => ValueOrArray<CockroachColumn | SQL | SQL.Aliased>]
			| (CockroachColumn | SQL | SQL.Aliased)[]
	): CockroachSelectWithout<this, TDynamic, 'groupBy'> {
		if (typeof columns[0] === 'function') {
			...
			this.config.groupBy = columns as (CockroachColumn | SQL | SQL.Aliased)[];
		}
		return this as any;
	}
	...
	orderBy(
		builder: (aliases: this['_']['selection']) => ValueOrArray<CockroachColumn | SQL | SQL.Aliased>,
	): CockroachSelectWithout<this, TDynamic, 'orderBy'>;
	...
	orderBy(
		...
		if (typeof columns[0] === 'function') {
			...
			const orderByArray = columns as (CockroachColumn | SQL | SQL.Aliased)[];
...
export interface CockroachSelectBase<
	TTableName extends string | undefined,
	TSelection extends ColumnsSelection,
	TSelectMode extends SelectMode,
	TNullabilityMap extends Record<string, JoinNullability> = TTableName extends string ? Record<TTableName, 'not-null'>
...
> extends CockroachSelectQueryBuilderBase<
	CockroachSelectHKT,
	TTableName,
	TSelection,
	TSelectMode,
...
function createSetOperator(type: SetOperator, isAll: boolean): CockroachCreateSetOperatorFn {
	return (leftSelect, rightSelect, ...restSelects) => {
		const setOperators = [rightSelect, ...restSelects].map((select) => ({
			type,
			isAll,
			rightSelect: select as AnyCockroachSelect,
		}));
		...
		return (leftSelect as AnyCockroachSelect).addSetOperators(setOperators) as any;
	};
}
