				...
				if (pathChunkIndex < path.length - 1) {
					...
					if (joinsNotNullableMap && path.length === 2) {
						...
						if (!(objectName in nullifyMap)) {
							nullifyMap[objectName] = (value === null && is(field, Column)) ? getTableName(field.table) : false;
						} else if (
							typeof nullifyMap[objectName] === 'string'
							&& (!is(field, Column) || nullifyMap[objectName] !== getTableName(field.table) || value !== null)
						) {
