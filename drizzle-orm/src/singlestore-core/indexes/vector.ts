...
type VectorIndexType = "AUTO" | "FLAT" | "IVF_FLAT" | "IVF_PQ" | "IVF_PQFS" | "HNSW_FLAT" | "HNSW_PQ";
type VectorMetricType = "EUCLIDEAN_DISTANCE" | "DOT_PRODUCT";

type VectorIndexConfigExtensions<T extends VectorIndexType> =
    T extends "IVF_FLAT"
    ? { nlist?: number; nprobe?: number }
    : T extends "IVF_PQ"
    ? { nlist?: number; m?: number; nbits?: number; nprobe?: number }
    : T extends "IVF_PQFS"
    ? { nlist?: number, m?: number, nprobe?: number }
    : T extends "HNSW_FLAT"
    ? { M?: number; efConstruction?: number; ef?: number }
    : T extends "HNSW_PQ"
    ? { M?: number; efConstruction?: number; m?: number; ef?: number; nbits?: number }
    : {};

interface VectorIndexConfigBase<T extends VectorIndexType> {
    name: string;
...
type VectorIndexConfig<T extends VectorIndexType> = VectorIndexConfigBase<T> & VectorIndexConfigExtensions<T>;

type VectorIndexBuilderMap<T extends VectorIndexType> = 
    T extends "AUTO" | "FLAT" ? VectorIndexBuilder<"AUTO" | "FLAT"> :
...
export function vectorIndex<T extends VectorIndexType>(name: string, indexType: T = "AUTO" as T): VectorIndexBuilderOn<T> {
