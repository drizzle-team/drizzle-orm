...
import type { DsqlAsyncDatabase } from '~/pg-core/async/dsql/db.ts';
 ...
 *
 * DSQL allows only one DDL statement per transaction and doesn't mix DDL with DML, so every statement runs on its own.
 */
...
export async function upgradeIfNeeded(
	...
	for (let v = version; v < MIGRATIONS_TABLE_VERSIONS.pg; v++) {
		...
		await upgradeFn(migrationsSchema, migrationsTable, db, localMigrations);
	}
