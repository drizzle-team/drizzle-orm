import { type QueryGenerator, YieldableQuery } from '~/generator-queries/generator.ts';
import type { MigrationMeta } from '~/migrator.ts';
import { type SQL, sql } from '~/sql/sql.ts';
import { GET_VERSION_FOR, MIGRATIONS_TABLE_VERSIONS } from './utils.ts';
 ...
 */
export function* upgradeIfNeeded(
	migrationsTable: string,
	localMigrations: MigrationMeta[],
) {
	// Check if the table exists at all
	const tableExists = yield* YieldableQuery.withResult(
	...
	for (let v = version; v < MIGRATIONS_TABLE_VERSIONS.sqlite; v++) {
		...
		yield* upgradeFn(migrationsTable, localMigrations);
	}
...
> = {
	 ...
	 */
	0: function*(migrationsTable, localMigrations) {
		...
		yield* YieldableQuery.batch(statements);
	},
};
