...
//  */
// export const upgradeIfNeeded = Effect.fn('upgradeIfNeeded')(
// 	function*(
...
// 	) {
// 		const tableExists = yield* session.all<{ exists: boolean }>(
// 			sql`SELECT EXISTS (
...
// 		const rows = yield* session.all<{ id: number; hash: string; created_at: string; version: number | undefined }>(
...
// 			const hasVersionColumn = yield* session.all<{ exists: boolean }>(
// 				sql`SELECT EXISTS (
...
// 			yield* runUpgrades(migrationsSchema, migrationsTable, session, prevVersion, localMigrations);
// 		}
...
//  */
// const runUpgrades = Effect.fn('runUpgrades')(function*(
// 	migrationsSchema: string,
// 	migrationsTable: string,
// 	session: PgEffectSession,
// 	fromVersion: number,
...
// 		if (!upgradeFn) {
// 			yield* Effect.fail(`No upgrade path from migration table version ${v} to ${v + 1}`);
// 		}
