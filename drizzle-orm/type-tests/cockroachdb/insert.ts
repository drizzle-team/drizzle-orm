...
import { boolean, cockroachdbTable, int4, QueryBuilder, text } from '~/cockroachdb-core/index.ts';
import type { CockroachDbInsert } from '~/cockroachdb-core/query-builders/insert.ts';
...
{
	function dynamic<T extends CockroachDbInsert>(qb: T) {
...
{
	function withReturning<T extends CockroachDbInsert>(qb: T) {
		return qb.returning();
	}
