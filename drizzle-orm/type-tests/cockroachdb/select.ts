...
import { alias } from '~/cockroachdb-core/alias.ts';
import {
	boolean,
	cockroachdbMaterializedView,
	type CockroachDbSelect,
	type CockroachDbSelectQueryBuilder,
	cockroachdbTable,
	cockroachdbView,
	int4,
	QueryBuilder,
...
{
	function withFriends<T extends CockroachDbSelect>(qb: T) {
		const friends = alias(users, 'friends');
...
{
	function withFriends<T extends CockroachDbSelectQueryBuilder>(qb: T) {
		const friends = alias(users, 'friends');
		const friends2 = alias(users, 'friends2');
		const friends3 = alias(users, 'friends3');
		const friends4 = alias(users, 'friends4');
...
{
	function dynamic<T extends CockroachDbSelect>(qb: T) {
...
{
	// TODO: add to docs
	function dynamic<T extends CockroachDbSelectQueryBuilder>(qb: T) {
		return qb.where(sql``).having(sql``).groupBy(sql``).orderBy(sql``).limit(1).offset(1).for('update');
	}
...
{
	// TODO: add to docs
	function paginated<T extends CockroachDbSelect>(qb: T, page: number) {
...
{
	const school = cockroachdbTable('school', {
		faculty: int4('faculty'),
		studentid: int4('studentid'),
	});

	const student = cockroachdbTable('student', {
		id: int4('id'),
		email: text('email'),
	});
...
{
	const table1 = cockroachdbTable('table1', {
		id: int4().primaryKey(),
		name: text().notNull(),
	});
	const table2 = cockroachdbTable('table2', {
		id: int4().primaryKey(),
		age: int4().notNull(),
	});
	const table3 = cockroachdbTable('table3', {
		id: int4().primaryKey(),
		phone: text().notNull(),
	});
	const view = cockroachdbView('view').as((qb) =>
		qb.select({
			table: table1,
			column: table2.age,
			nested: {
...

{
	...
	const view = cockroachdbMaterializedView('view').as((qb) =>
		qb.select({
			table: table1,
			column: table2.age,
			nested: {
...

{
	...
	const table2 = cockroachdbTable('table2', {
		id: int4().primaryKey(),
		age: int4().notNull(),
		table1Id: int4().references(() => table1.id).notNull(),
	});

	const view = cockroachdbView('view').as((qb) => qb.select().from(table2));
