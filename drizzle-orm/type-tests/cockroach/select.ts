...
{
	function dynamic<T extends CockroachSelect>(qb: T) {
...
{
	// TODO: add to docs
	function dynamic<T extends CockroachSelectQueryBuilder>(qb: T) {
		return qb.where(sql``).having(sql``).groupBy(sql``).orderBy(sql``).limit(1).offset(1).for('update');
	}
...
{
	// TODO: add to docs
	function paginated<T extends CockroachSelect>(qb: T, page: number) {
...
{
	const school = cockroachTable('school', {
		faculty: int4('faculty'),
		studentid: int4('studentid'),
	});

	const student = cockroachTable('student', {
		id: int4('id'),
		email: text('email'),
	});
...

{
	...
	const table3 = cockroachTable('table3', {
		id: int4().primaryKey(),
		phone: text().notNull(),
	});
	const view = cockroachView('view').as((qb) =>
		qb.select({
			table: table1,
			column: table2.age,
			nested: {
...

{
	...
	const view = cockroachMaterializedView('view').as((qb) =>
		qb.select({
			table: table1,
			column: table2.age,
			nested: {
...

{
	...
	const view = cockroachView('view').as((qb) => qb.select().from(table2));
