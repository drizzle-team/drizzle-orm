...
import type { AnyCockroachMaterializedView, AnyCockroachView } from '~/cockroach-core/view.ts';
...
const view = cockroachView('view').as((qb) => qb.select().from(table));
const manualView = cockroachView('manual_view', {
	id: int4('id').notNull(),
	name: text('name'),
}).as(sql`select * from ${table}`);
const existingView = cockroachView('existing_view', {
	id: int4('id').notNull(),
	name: text('name'),
}).existing();
...
const materializedView = cockroachMaterializedView('materialized_view').as((qb) => qb.select().from(table));
const schemaMaterializedView = mySchema.materializedView('schema_materialized_view').as((qb) =>
	qb.select().from(schemaTable)
);
...

{
	...
	Expect<Equal<typeof materializedView['_']['isAlias'], false>>;
	Expect<Equal<typeof schemaMaterializedView['_']['isAlias'], false>>;
}
...
{
	const aliased = alias(materializedView, 'aliased_materialized_view');

	Expect<Equal<typeof aliased['_']['name'], 'aliased_materialized_view'>>;
	Expect<Equal<typeof aliased['_']['isAlias'], true>>;
	Expect<Equal<typeof aliased['_']['schema'], undefined>>;
	Expect<Equal<typeof aliased['_']['existing'], false>>;
...
{
	const aliased = alias(schemaMaterializedView, 'aliased_schema_materialized_view');

	Expect<Equal<typeof aliased['_']['name'], 'aliased_schema_materialized_view'>>;
	Expect<Equal<typeof aliased['_']['isAlias'], true>>;
	Expect<Equal<typeof aliased['_']['schema'], 'my_schema'>>;
	Expect<Equal<typeof aliased['_']['existing'], false>>;
...

{
	...
	const aliasedMaterializedView = alias(materializedView, 'aliased_materialized_view');

	const result = await db
		.select()
		.from(aliasedView)
		.innerJoin(aliasedExistingView, sql`true`)
		.leftJoin(aliasedMaterializedView, sql`true`);
	Expect<
		Equal<typeof result, {
			aliased_view: SelectedRow;
			aliased_existing_view: SelectedRow;
			aliased_materialized_view: SelectedRow | null;
		}[]>
	>;
}

{
	...
	const asView: AnyCockroachView = aliasedView;
	const asMaterialized: AnyCockroachMaterializedView = aliasedMaterializedView;

	// No error - `CockroachMaterializedView` doesn't define own properties, flip the expectation if fields are ever added
	const materializedAsView: AnyCockroachView = aliasedMaterializedView;
	// @ts-expect-error an aliased plain view is not a materialized view
	const viewAsMaterialized: AnyCockroachMaterializedView = aliasedView;

	void asView;
	void asMaterialized;
	void materializedAsView;
	void viewAsMaterialized;
}
