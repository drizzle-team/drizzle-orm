...
import { integer, pgMaterializedView, pgSchema, pgTable, pgView, text } from '~/pg-core/index.ts';
import type { AnyPgMaterializedView, AnyPgView } from '~/pg-core/view.ts';
...
const view = pgView('view').as((qb) => qb.select().from(table));
const manualView = pgView('manual_view', {
	id: integer('id').notNull(),
	name: text('name'),
}).as(sql`select * from ${table}`);
const existingView = pgView('existing_view', {
	id: integer('id').notNull(),
	name: text('name'),
}).existing();
...
const materializedView = pgMaterializedView('materialized_view').as((qb) => qb.select().from(table));
...

{
	...
	const asView: AnyPgView = aliasedView;
	const asMaterialized: AnyPgMaterializedView = aliasedMaterializedView;

	// @ts-expect-error an aliased materialized view is not a plain view
	const materializedAsView: AnyPgView = aliasedMaterializedView;
	// @ts-expect-error an aliased plain view is not a materialized view
	const viewAsMaterialized: AnyPgMaterializedView = aliasedView;
