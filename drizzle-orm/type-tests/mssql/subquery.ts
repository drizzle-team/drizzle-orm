...
import { alias, int, mssqlTable, text } from '~/mssql-core/index.ts';
import type { MsSqlSelect } from '~/mssql-core/query-builders/select.types.ts';
import { and, eq } from '~/sql/expressions';
import { sql } from '~/sql/sql.ts';
import type { DrizzleTypeError, Equal } from '~/utils.ts';
import { db } from './db.ts';

const names = mssqlTable('names', {
...
// https://github.com/drizzle-team/drizzle-orm/issues/4069
{
	...
	const countOfGeneric = async <T extends MsSqlSelect>(qb: T) =>
		db.select({ count: sql<number>`count(1)` }).from(qb.as('generic_sq'));

	const joinGeneric = async <T extends MsSqlSelect>(qb: T) =>
	...
	const scalarGeneric = async <T extends MsSqlSelect>(qb: T) =>
