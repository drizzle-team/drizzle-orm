...
import type { MySqlSelect } from '~/mysql-core/query-builders/select.ts';
...
// https://github.com/drizzle-team/drizzle-orm/issues/4069
{
	...
	const countOfGeneric = async <T extends MySqlSelect>(qb: T) =>
		db.select({ count: sql<number>`count(1)` }).from(qb.as('generic_sq'));

	const joinGeneric = async <T extends MySqlSelect>(qb: T) =>
	...
	const scalarGeneric = async <T extends MySqlSelect>(qb: T) =>
