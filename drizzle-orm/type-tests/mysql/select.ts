...
const allOperators = await db
	...
	.where(and(
		...
		between(users.id, 1, sql`2`),
		between(1, users.id, sql`2`),
		between(sql`2`, 1, users.id),
		notBetween(users.id, 1, sql`2`),
		notBetween(1, users.id, sql`2`),
		notBetween(sql`2`, 1, users.id),
		like(users.id, '%1%'),
		notLike(users.id, '%1%'),
		ilike(users.id, '%1%'),
		notIlike(users.id, '%1%'),
