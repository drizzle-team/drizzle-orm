...
// TODO - SPANNER - verify
const alteredGoogleSqlViewSchema = alteredViewCommon.merge(
	object({
		alteredMeta: object({
			__old: string(),
			__new: string(),
...
export const diffResultSchemeGooglesql = object({
	alteredTablesWithColumns: alteredTableScheme.array(),
	alteredEnums: never().array(),
	alteredViews: alteredGoogleSqlViewSchema.array(),
});
...
export type DiffResulGooglesql = TypeOf<typeof diffResultSchemeGooglesql>;
...
// TODO - SPANNER - verify
export const applyGooglesqlSnapshotsDiff = async (
	json1: GoogleSqlSchemaSquashed,
	json2: GoogleSqlSchemaSquashed,
	tablesResolver: (
		input: ResolverInput<Table>,
	) => Promise<ResolverOutputWithMoved<Table>>,
	columnsResolver: (
	...
	prevFull: GoogleSqlSchema,
	curFull: GoogleSqlSchema,
	action?: 'push' | undefined,
): Promise<{
	statements: JsonStatement[];
	sqlStatements: string[];
	...
	const typedResult: DiffResulGooglesql = diffResultSchemeGooglesql.parse(diffResult);
	...
	const jsonTableAlternations = alteredTables
		.map((it) => {
			return prepareAlterColumnsGooglesql(
				it.name,
				it.schema,
				it.altered,
				json1,
	...
	const jsonGoogleSqlCreateTables = createdTables.map((it) => {
		return prepareGoogleSqlCreateTableJson(
			it,
			curFull as GoogleSqlSchema,
			curFull.internal,
		);
	});

	const createViews: JsonCreateGoogleSqlViewStatement[] = [];
	const dropViews: JsonDropViewStatement[] = [];
	const renameViews: JsonRenameViewStatement[] = [];
	const alterViews: JsonAlterGoogleSqlViewStatement[] = [];

	createViews.push(
		...createdViews.filter((it) => !it.isExisting).map((it) => {
			return prepareGoogleSqlCreateViewJson(
				it.name,
				it.definition!,
				it.meta,
			);
	...
	for (const alteredView of alteredViews) {
		...
		if (alteredView.alteredMeta) {
			const view = curFull['views'][alteredView.name];
			alterViews.push(
				prepareGoogleSqlAlterView(view),
			);
		}
	}

	jsonStatements.push(...jsonGoogleSqlCreateTables);
	...
	const sqlStatements = fromJson(jsonStatements, 'googlesql');
