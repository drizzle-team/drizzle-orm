...
import {
	...
	prepareAddCompositePrimaryKeyMsSql,
	...
	prepareAlterColumnsMssql,
	prepareAlterColumnsMysql,
	prepareAlterCompositePrimaryKeyMsSql,
	...
	prepareDeleteCompositePrimaryKeyMsSql,
	...
	prepareMsSqlAlterView,
	prepareMsSqlCreateTableJson,
...
import { MsSqlSchema, MsSqlSchemaSquashed, MsSqlSquasher } from './serializer/mssqlSchema';
...
export const diffResultSchemeMssql = object({
	alteredTablesWithColumns: alteredTableScheme.array(),
	alteredEnums: never().array(),
	alteredViews: alteredMySqlViewSchema.array(),
});
...
export type DiffResultMssql = TypeOf<typeof diffResultSchemeMssql>;
...
export const applyMssqlSnapshotsDiff = async (
	json1: MsSqlSchemaSquashed,
	json2: MsSqlSchemaSquashed,
	tablesResolver: (
		input: ResolverInput<Table>,
	) => Promise<ResolverOutputWithMoved<Table>>,
	columnsResolver: (
	...
	// squash uniqueIndexes and uniqueConstraint into constraints object
	// it should be done for mssql only because it has no diffs for it

	for (const tableName in json1.tables) {
		const table = json1.tables[tableName];
		for (const indexName in table.indexes) {
			const index = MsSqlSquasher.unsquashIdx(table.indexes[indexName]);
			if (index.isUnique) {
				table.uniqueConstraints[indexName] = MsSqlSquasher.squashUnique({
					name: index.name,
					columns: index.columns,
				});
				delete json1.tables[tableName].indexes[index.name];
	...
	const diffResult = applyJsonDiff(viewsPatchedSnap1, json2);
	const typedResult: DiffResultMssql = diffResultSchemeMssql.parse(diffResult);
	...
	alteredTables.forEach((it) => {
		...
		addedCompositePKs = prepareAddCompositePrimaryKeyMsSql(
			it.name,
			it.addedCompositePKs,
			prevFull,
			curFull,
		);
		deletedCompositePKs = prepareDeleteCompositePrimaryKeyMsSql(
			it.name,
			it.deletedCompositePKs,
			prevFull,
		);
		// }
		alteredCompositePKs = prepareAlterCompositePrimaryKeyMsSql(
			it.name,
			it.alteredCompositePKs,
			prevFull,
			curFull,
	...
	const jsonTableAlternations = alteredTables
		.map((it) => {
			return prepareAlterColumnsMssql(
				it.name,
				it.schema,
				it.altered,
				json1,
	...
	const jsonMsSqlCreateTables = createdTables.map((it) => {
		return prepareMsSqlCreateTableJson(
			it,
			curFull as MsSqlSchema,
			curFull.internal,
		);
	});
	...
	const dropViews: JsonDropViewStatement[] = [];
	const renameViews: JsonRenameViewStatement[] = [];
	const alterViews: JsonAlterMsSqlViewStatement[] = [];
	...
	for (const alteredView of alteredViews) {
		...
		if (alteredView.alteredMeta) {
			const view = curFull['views'][alteredView.name];
			alterViews.push(
				prepareMsSqlAlterView(view),
			);
		}
	}

	jsonStatements.push(...jsonMsSqlCreateTables);
