import { originUUID, snapshotVersion, mapValues } from "../global";
...
export const PgSquasher = {
  squashIdx: (idx: Index) => {
    ...
    return `${idx.name};${idx.columns
      .map(
        (c) =>
          `${c.expression}--${c.isExpression}--${c.asc}--${c.nulls}--${c.opclass}`
      )
      .join(",,")};${idx.isUnique};${idx.concurrently};${idx.method};${
      idx.where
  ...
  squashFK: (fk: ForeignKey) => {
    return `${fk.name};${fk.tableFrom};${fk.columnsFrom.join(",")};${
      fk.tableTo
  ...
  squashSequence: (seq: Omit<Sequence, "name" | "schema">) => {
    return `${seq.minValue};${seq.maxValue};${seq.increment};${seq.startWith};${
      seq.cache
    };${seq.cycle ?? ""}`;
  },
  ...
  squashIdentity: (
    ...
    return `${seq.name};${seq.type};${seq.minValue};${seq.maxValue};${
      seq.increment
    };${seq.startWith};${seq.cache};${seq.cycle ?? ""}`;
  },
  unsquashIdentity: (
    seq: string
