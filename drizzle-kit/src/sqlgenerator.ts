  ...
  convert(st: JsonCreateTableStatement) {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      const uniqueConstraint = column.isUnique
        ? ` CONSTRAINT "${column.uniqueName}" UNIQUE${
            column.nullsNotDistinct ? " NULLS NOT DISTINCT" : ""
          }`
        : "";
      ...
      statement +=
        ...
        `"${
          column.name
        }" ${type}${primaryKeyStatement}${defaultStatement}${notNullStatement}${uniqueConstraint}${
    ...
    if (typeof compositePKs !== "undefined" && compositePKs.length > 0) {
      ...
      statement += `\tCONSTRAINT "${
        st.compositePkName
      }" PRIMARY KEY(\"${compositePK.columns.join(`","`)}\")`;
      // statement += `\n`;
    }
...
export class SQLiteCreateTableConvertor extends Convertor {
  ...
  convert(st: JsonSqliteCreateTableStatement) {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      statement += "\t";
      statement += `\`${column.name}\` ${column.type}${primaryKeyStatement}${autoincrementStatement}${defaultStatement}${notNullStatement}${generatedStatement}`;
    ...
    if (
      ...
      for (const uniqueConstraint of uniqueConstraints) {
        ...
        } UNIQUE(\`${unsquashedUnique.columns.join(`\`,\``)}\`)`;
      }
    }
...
class PgAlterTableAddUniqueConstraintConvertor extends Convertor {
  ...
  convert(statement: JsonCreateUniqueConstraint): string {
    ...
    return `ALTER TABLE ${tableNameWithSchema} ADD CONSTRAINT "${
      unsquashed.name
    }" UNIQUE${
...
class MySQLAlterTableAddUniqueConstraintConvertor extends Convertor {
  ...
  convert(statement: JsonCreateUniqueConstraint): string {
    ...
    return `ALTER TABLE \`${statement.tableName}\` ADD CONSTRAINT \`${
      unsquashed.name
    }\` UNIQUE(\`${unsquashed.columns.join("`,`")}\`);`;
  }
}
...
class CreatePgSequenceConvertor extends Convertor {
  ...
  convert(st: JsonCreateSequenceStatement) {
    ...
    return `CREATE SEQUENCE ${sequenceWithSchema}${
      values.increment ? ` INCREMENT BY ${values.increment}` : ""
...
class AlterPgSequenceConvertor extends Convertor {
  ...
  convert(st: JsonAlterSequenceStatement) {
    ...
    return `ALTER SEQUENCE ${sequenceWithSchema}${
    ...
    if (statement.type === "alter_table_alter_column_drop_notnull") {
      ...
      if (statement.columnGenerated?.type === "virtual") {
        ...
        columnGenerated = statement.columnGenerated
          ? ` GENERATED ALWAYS AS (${
              statement.columnGenerated?.as
            }) ${statement.columnGenerated?.type.toUpperCase()}`
          : "";
      }
...
class MySqlAlterTableAlterCompositePrimaryKeyConvertor extends Convertor {
  ...
  convert(statement: JsonAlterCompositePK) {
    ...
    return `ALTER TABLE \`${
      statement.tableName
    }\` DROP PRIMARY KEY, ADD PRIMARY KEY(\`${newColumns.join("`,`")}\`);`;
  }
}
  ...
  convert(statement: JsonAlterColumnDropPrimaryKeyStatement) {
    ...
    Meanwhile you can:
        1. Check pk name in your database, by running
            ...
            WHERE table_schema = '${
              typeof schema === "undefined" || schema === "" ? "public" : schema
            }'
