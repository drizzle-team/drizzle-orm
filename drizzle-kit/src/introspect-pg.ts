...
export const relationsToTypeScriptForStudio = (
  ...
  Object.values(relationsConfig.tables).forEach((table) => {
    ...
    if (hasRelations) {
      result += `export const ${tableName}Relation = relations(${tableName}, ({${
        hasOne ? "one" : ""
      }${hasOne && hasMany ? ", " : ""}${hasMany ? "many" : ""}}) => ({
        ${relationsObjAsStr}
      }));\n`;
    }
  });
...
export const schemaToTypeScript = (
  ...
  const enumStatements = Object.values(schema.enums)
    .map((it) => {
      ...
      const values = Object.values(it.values)
        .map((it) => `'${it}'`)
        .join(", ");
      return `export const ${withCasing(paramName, casing)} = ${func}("${
        it.name
      }", [${values}])\n`;
    })
    .join("")
  ...
  const tableStatements = Object.values(schema.tables).map((table) => {
    ...
    let statement = `export const ${withCasing(paramName, casing)} = ${func}("${
      table.name
    }", {\n`;
  ...
  if (lowered.startsWith("geometry")) {
    ...
    if (lowered.length !== 8) {
      ...
      if (geometryOptions.length === 1 && geometryOptions[0] !== "") {
        out = `${withCasing(name, casing)}: geometry("${name}", { type: "${
          geometryOptions[0]
        }" })`;
