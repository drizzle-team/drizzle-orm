...
import { MsSqlKitInternals, MsSqlSchema, MsSqlSquasher, View as MsSqlView } from './serializer/mssqlSchema';
...
export type JsonCreateMsSqlViewStatement = {
	type: 'mssql_create_view';
	replace: boolean;
} & Omit<MsSqlView, 'columns' | 'isExisting'>;
...
export type JsonAlterMsSqlViewStatement = {
	type: 'alter_mssql_view';
} & Omit<MsSqlView, 'isExisting'>;
...
export const prepareMsSqlCreateTableJson = (
	table: Table,
	// TODO: remove?
	json2: MsSqlSchema,
	// we need it to know if some of the indexes(and in future other parts) are expressions or columns
...
export const prepareAlterColumnsMssql = (
	tableName: string,
	schema: string,
	columns: AlteredColumn[],
	// TODO: remove?
	...
	for (const column of columns) {
		...
		// I used any, because those fields are available only for mssql dialect
		// For other dialects it will become undefined, that is fine for json statements
		const columnType = json2.tables[tableName].columns[columnName].type;
		const columnDefault = json2.tables[tableName].columns[columnName].default;
		const columnGenerated = json2.tables[tableName].columns[columnName].generated;
...
export const prepareAddCompositePrimaryKeyMsSql = (
	tableName: string,
	pks: Record<string, string>,
	// TODO: remove?
	json1: MsSqlSchema,
	...
	const res: JsonCreateCompositePK[] = [];
	for (const it of Object.values(pks)) {
		const unsquashed = MsSqlSquasher.unsquashPK(it);
...
export const prepareDeleteCompositePrimaryKeyMsSql = (
	tableName: string,
	pks: Record<string, string>,
	// TODO: remove?
	json1: MsSqlSchema,
...
export const prepareAlterCompositePrimaryKeyMsSql = (
	tableName: string,
	pks: Record<string, { __old: string; __new: string }>,
	// TODO: remove?
	json1: MsSqlSchema,
	...
	return Object.values(pks).map((it) => {
		return {
			...
			new: it.__new,
			oldConstraintName: json1.tables[tableName].compositePrimaryKeys[
				MsSqlSquasher.unsquashPK(it.__old).name
			].name,
			newConstraintName: json2.tables[tableName].compositePrimaryKeys[
				MsSqlSquasher.unsquashPK(it.__new).name
			].name,
		} as JsonAlterCompositePK;
	});
};
...
export const prepareMsSqlAlterView = (
	view: Omit<MsSqlView, 'isExisting'>,
): JsonAlterMsSqlViewStatement => {
	return { type: 'alter_mssql_view', ...view };
};
