...
export const preparePgCreateTableJson = (
  ...
  const { name, schema, columns, compositePrimaryKeys, uniqueConstraints } =
    table;
...
export const _prepareSqliteAddColumns = (
  ...
  const unsquashed = referenceData.map((addedFkValue) =>
    SQLiteSquasher.unsquashFK(addedFkValue)
  );

  return columns
    .map((it) => {
      const columnsWithReference = unsquashed.find((t) =>
        t.columnsFrom.includes(it.name)
      );
...
export const prepareAlterColumnsMysql = (
  ...
  for (const column of columns) {
    ...
    const columnOnUpdate =
      "onUpdate" in snapshotColumn ? snapshotColumn.onUpdate : undefined;
    const columnNotNull = table.columns[columnName].notNull;

    const columnAutoIncrement =
      "autoincrement" in snapshotColumn
  ...
  for (const column of columns) {
    ...
    const columnGenerated =
      json2.tables[tableName].columns[columnName].generated;
    ...
    if (column.generated?.type === "added") {
      if (columnGenerated?.type === "virtual") {
        warning(
          `You are trying to add virtual generated constraint to ${chalk.blue(
            columnName
    ...
    if (column.generated?.type === "deleted") {
      if (columnGenerated?.type === "virtual") {
        warning(
          `You are trying to remove virtual generated constraint from ${chalk.blue(
            columnName
...
export const preparePgAlterColumns = (
  ...
  for (const column of columns) {
    ...
    const columnGenerated =
      json2.tables[tableKey].columns[columnName].generated;
    ...
    const compositePk =
      json2.tables[tableKey].compositePrimaryKeys[`${tableName}_${columnName}`];
...
export const prepareAddCompositePrimaryKeyPg = (
  ...
  return Object.values(pks).map((it) => {
    ...
    return {
      ...
      schema,
      constraintName:
        json2.tables[`${schema || "public"}.${tableName}`].compositePrimaryKeys[
          unsquashed.name
        ].name,
    } as JsonCreateCompositePK;
  });
...
export const prepareDeleteCompositePrimaryKeyPg = (
  ...
  return Object.values(pks).map((it) => {
    return {
      ...
      schema,
      constraintName:
        json1.tables[`${schema || "public"}.${tableName}`].compositePrimaryKeys[
...
export const prepareAddCompositePrimaryKeyMySql = (
  ...
  for (const it of Object.values(pks)) {
    ...
    res.push({
      ...
      data: it,
      constraintName:
        json2.tables[tableName].compositePrimaryKeys[unsquashed.name].name,
    } as JsonCreateCompositePK);
  }
  return res;
};
...
export const prepareDeleteCompositePrimaryKeyMySql = (
  ...
  return Object.values(pks).map((it) => {
    return {
      ...
      data: it,
      constraintName:
        json1.tables[tableName].compositePrimaryKeys[
