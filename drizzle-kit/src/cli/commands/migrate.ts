...
export const tablesResolver = async (
  ...
  try {
    const { created, deleted, moved, renamed } =
      await promptNamedWithSchemasConflict(
        input.created,
        input.deleted,
        "table"
      );
        ...
        suggestions.push(
          `\n` +
            withStyle.suggestion(
              `We are suggesting to change ${chalk.blue(
                column.name
              )} column in ${chalk.blueBright(
                table.name
              )} table from serial to bigint unsigned\n\n${chalk.blueBright(
                `bigint("${
                  column.name
                }", { mode: "number", unsigned: true }).notNull().autoincrement().unique(${
...
export const promptColumnsConflicts = async <T extends Named>(
  ...
  do {
    ...
    if (isRenamePromptItem(data)) {
      console.log(
        `${chalk.yellow("~")} ${data.from.name} › ${data.to.name} ${chalk.gray(
      ...
      console.log(
        `${chalk.green("+")} ${data.name} ${chalk.gray(
...
export const promptNamedWithSchemasConflict = async <T extends NamedWithSchema>(
  ...
  do {
    ...
    if (isRenamePromptItem(data)) {
      const schemaFromPrefix =
        !data.from.schema || data.from.schema === "public"
          ? ""
          : `${data.from.schema}.`;
      const schemaToPrefix =
        !data.to.schema || data.to.schema === "public"
          ? ""
          : `${data.to.schema}.`;

      console.log(
        `${chalk.yellow("~")} ${schemaFromPrefix}${
          data.from.name
        } › ${schemaToPrefix}${data.to.name} ${chalk.gray(
...
export const writeResult = ({
  ...
  const idx =
    typeof lastEntryInJournal === "undefined" ? 0 : lastEntryInJournal.idx + 1;
  ...
  render(
    `[${chalk.green(
      "✓"
    )}] Your SQL migration file ➜ ${chalk.bold.underline.blue(
...
export const embeddedMigrations = (journal: Journal) => {
  ...
  journal.entries.forEach((entry) => {
    content += `import m${entry.idx.toString().padStart(4, "0")} from './${
      entry.tag
    }.sql';\n`;
  });
