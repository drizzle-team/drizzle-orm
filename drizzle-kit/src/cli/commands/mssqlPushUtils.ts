...
import { mssqlSchema, MsSqlSquasher } from 'src/serializer/mssqlSchema';
...
export const filterStatements = (
	...
	currentSchema: TypeOf<typeof mssqlSchema>,
	prevSchema: TypeOf<typeof mssqlSchema>,
) => {
	return statements.filter((statement) => {
		if (statement.type === 'alter_table_alter_column_set_type') {
			...
			const unsquashed = MsSqlSquasher.unsquashUnique(statement.data);
			...
			//     MsSqlSquasher.unsquashUnique(statement.data).columns[0]
			// ) {
			//   return false;
			// }
...
export const logSuggestionsAndReturn = async (
	db: DB,
	statements: JsonStatement[],
	json2: TypeOf<typeof mssqlSchema>,
) => {
	...
	for (const statement of statements) {
		if (statement.type === 'drop_table') {
			...
			const count = Number(res[0].count);
			if (count > 0) {
				const unsquashedUnique = MsSqlSquasher.unsquashUnique(statement.data);
				console.log(
					`· You're about to add ${
						chalk.underline(
