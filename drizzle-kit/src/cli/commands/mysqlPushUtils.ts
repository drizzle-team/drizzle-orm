...
import { MySqlSquasher, mysqlSchema } from "../../serializer/mysqlSchema";
...
export const logSuggestionsAndReturn = async (
  ...
  for (const statement of statements) {
    if (statement.type === "drop_table") {
      ...
      if (count > 0) {
        infoToPrint.push(
          `· You're about to delete ${chalk.underline(
            statement.tableName
      ...
      if (count > 0) {
        infoToPrint.push(
          `· You're about to change ${chalk.underline(
            statement.columnName
          )} column type from ${chalk.underline(
      ...
      if (statement.columnNotNull) {
        ...
        if (count > 0) {
          infoToPrint.push(
            `· You're about to remove default value from ${chalk.underline(
              statement.columnName
      ...
      if (typeof statement.columnDefault === "undefined") {
        ...
        if (count > 0) {
          infoToPrint.push(
            `· You're about to set not-null constraint to ${chalk.underline(
              statement.columnName
      ...
      if (
        ...
        if (count > 0) {
          infoToPrint.push(
            `· You're about to add not-null ${chalk.underline(
      ...
      if (count > 0) {
        ...
        console.log(
          `· You're about to add ${chalk.underline(
            unsquashedUnique.name
          )} unique constraint to the table, which contains ${count} items. If this statement fails, you will receive an error from the database. Do you want to truncate ${chalk.underline(
            statement.tableName
          )} table?\n`
        );
