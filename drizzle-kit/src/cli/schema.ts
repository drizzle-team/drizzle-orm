...
import { MigrateProgress, grey } from "./views";
import { mkdirSync } from "fs";
import { renderWithTask } from "hanji";
import { command, string, boolean, number } from "@drizzle-team/brocli";
...
export const pull = command({
  ...
  handler: async (config) => {
    ...
    console.log(
      grey(
        `Pulling from [${schemasFilter
          .map((it) => `'${it}'`)
          .join(", ")}] list of schemas`
      )
    );
    console.log();
      ...
      server.start({
        ...
        cb: (err, address) => {
          if (err) {
            ...
            console.log(
              `\nDrizzle Studio is up and running on ${chalk.blue(
