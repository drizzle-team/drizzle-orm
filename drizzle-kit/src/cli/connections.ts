...
> => {
	if ('driver' in credentials) {
		...
		if (driver === 'aws-data-api') {
			...
			const { RDSDataClient, ExecuteStatementCommand, TypeHint } = await import('@aws-sdk/client-rds-data');
			...
			const session = new AwsDataApiSession(rdsClient, new PgDialect(), undefined, undefined, config, undefined);
			...
			const query = async (sql: string, params: any[]) => {
				const prepared = session.prepareQuery({ sql, params: params ?? [] }, undefined, undefined, false);
	...
	if (await checkPackage('pg')) {
		...
		const ssl = 'ssl' in credentials
			? credentials.ssl === 'prefer' || credentials.ssl === 'require' || credentials.ssl === 'allow'
	...
	if (await checkPackage('postgres')) {
		console.log(withStyle.info(`Using 'postgres' driver for database querying`));
	...
	if (await checkPackage('@vercel/postgres')) {
		console.log(withStyle.info(`Using '@vercel/postgres' driver for database querying`));
		console.log(
	...
	if (await checkPackage('@neondatabase/serverless')) {
		console.log(withStyle.info(`Using '@neondatabase/serverless' driver for database querying`));
		console.log(
			withStyle.fullWarning(
...
export const connectToSingleStore = async (
	...
	if (await checkPackage('mysql2')) {
		...
		const connection = result.url ? await createConnection(result.url) : await createConnection(result.credentials!); // needed for some reason!
		...
		const query: DB['query'] = async <T>(sql: string, params?: any[]): Promise<T[]> => {
	...
	console.error("To connect to SingleStore database - please install 'singlestore' driver");
	process.exit(1);
};
...
const prepareSqliteParams = (params: any[], driver?: string) => {
	return params.map((param) => {
		if (param && typeof param === 'object' && 'type' in param && 'value' in param && param.type === 'binary') {
			const value = typeof param.value === 'object' ? JSON.stringify(param.value) : (param.value as string);
...
> => {
	if ('driver' in credentials) {
		...
		if (driver === 'd1-http') {
			...
			const remoteCallback: Parameters<typeof drizzle>[0] = async (sql, params, method) => {
			...
			// TODO: Implement transaction for d1-http
			const transaction = async (queries: ProxyParams[]) => {
				const results: any[] = [];

				for (const query of queries) {
					const result = await proxy.proxy(query);
					results.push(result);
				}
			...
			return { ...db, ...proxy, transaction, migrate: migrateFn };
		} else {
			assertUnreachable(driver);
		}
	}
	...
	if (await checkPackage('@libsql/client')) {
		...
		const transaction = async (queries: ProxyParams[]) => {
			...
			// const results = await client.batch(queries.map((query) => ({ sql: query.sql, args: query.params || [] })));
	...
	if (await checkPackage('better-sqlite3')) {
		...
		const sqlite = new Database(normaliseSQLiteUrl(credentials.url, 'better-sqlite'));
		...
		const transaction = async (queries: ProxyParams[]) => {
			...
			const tx = sqlite.transaction(async (queries: ProxyParams[]) => {
				for (const query of queries) {
					const result = await proxy.proxy(query);
					results.push(result);
				}
...
): Promise<LibSQLDB & SqliteProxy & { migrate: (config: MigrationConfig) => Promise<void> }> => {
	if (await checkPackage('@libsql/client')) {
		const { createClient } = await import('@libsql/client');
		const { drizzle } = await import('drizzle-orm/libsql');
		const { migrate } = await import('drizzle-orm/libsql/migrator');
