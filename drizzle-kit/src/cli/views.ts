...
export const info = (msg: string, greyMsg: string = ""): string => {
  return `${chalk.blue.bold("Info:")} ${msg} ${
    greyMsg ? chalk.grey(greyMsg) : ""
  }`.trim();
};
...
export const error = (error: string, greyMsg: string = ""): string => {
  return `${chalk.bgRed.bold(" Error ")} ${error} ${
    greyMsg ? chalk.grey(greyMsg) : ""
  }`.trim();
};
...
export const schema = (schema: CommonSchema): string => {
  ...
  msg += tables
    .map((t) => {
      ...
      return `${chalk.bold.blue(t.name)} ${chalk.gray(
  ...
  if (enums.length > 0) {
    ...
    msg += enums
      .map((it) => {
        return `${chalk.bold.blue(it.name)} ${chalk.gray(
...
> {
  ...
  render(status: "idle" | "submitted" | "aborted"): string {
    ...
    let text = `\nIs ${chalk.bold.blue(key)} ${
      this.entityType
    } created or renamed from another ${this.entityType}?\n`;
