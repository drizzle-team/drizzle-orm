...
export const createDDL = () => {
	return create({
		...
		identity: {
			...
			privileges: (r) => JSON.stringify([r.schema, r.table, r.grantor, r.grantee, r.type]),
		...
		edges: {
			...
			privileges: [{ to: 'tables', map: { schema: 'schema', name: 'table' } }],
			columns: [
			...
			// an fk belongs to its table and its own columns, and references another table plus
			// that table's columns (columnsTo).
			fks: [
				{ to: 'tables', map: { schema: 'schema', name: 'table' } },
				{ to: 'columns', map: { schema: 'schema', table: 'table', name: { list: 'columns' } } },
...
export type PostgresEntities = NonNullable<PostgresDDL['$entities']>;
export type PostgresEntity = PostgresEntities[keyof PostgresEntities];

export type DiffEntities = { [K in keyof PostgresEntities]: Alter<PostgresEntities[K]> };
...
export const interimToDDL = (
	...
	for (const it of schema.privileges) {
		...
		if (res.status === 'CONFLICT') {
			// Privileges have no DB name; label the duplicate by its identity for the message.
			errors.push({
				type: 'privilege_duplicate',
				name: `${it.grantor}_${it.grantee}_${it.type}`,
			});
		}
	}
