...
export const fromDatabase = async (
  ...
  `).then((rows) => {
	...
	for (const column of columnsList.filter((x) => x.kind === 'r' || x.kind === 'p')) {
		...
		const sequence = column.seqId
			? sequencesList.find((it) => Number(it.oid) === Number(column.seqId))
			: null;
		...
		columns.push({
			...
			generated: column.generatedType === 's' && columnDefault?.expression
				? { type: 'stored', as: columnDefault.expression }
				: null,
			identity: column.identityType !== '' && sequence
				? {
					type: column.identityType === 'a' ? 'always' : 'byDefault',
					name: sequence.name,
					increment: parseIdentityProperty(sequence.incrementBy),
					minValue: parseIdentityProperty(sequence.minValue),
					maxValue: parseIdentityProperty(sequence.maxValue),
					startWith: parseIdentityProperty(sequence.startWith),
					cycle: sequence.cycle,
					cache: Number(parseIdentityProperty(sequence.cacheSize)),
				}
				: null,
		});
	}
	...
	for (const idx of idxs) {
		const metadata: IndexMetadata = typeof idx.metadata === 'string' ? JSON.parse(idx.metadata) : idx.metadata;
