...
class ImportAnalyzer {
	...
	private isCustomLocal = (importTarget: string) =>
		!!this.localPaths.find((l) => importTarget === l || importTarget.startsWith(l.endsWith('/') ? l : `${l}/`));
	private isLocal = (importTarget: string) =>
		this.localImportRegex.test(importTarget) || this.isCustomLocal(importTarget);
	private isTsFormat = (path: string) => this.importedFileFormatRegex.test(path);

	private resolveCustomLocalPath = (absoluteBase: string, base: string, target: string): string => {
	...
	private resolveTargetFile = (path: string): string => {
		...
		const formats = ['.ts', '.mts', '.cts', '.tsx', '.js', '.mjs', '.cjs', '.jsx'];
	...
	private resolveTargetPath = (absoluteBase: string, base: string, target: string): string => {
		if (this.isCustomLocal(target)) {
			return this.resolveTargetFile(this.resolveCustomLocalPath(absoluteBase, base, target));
		}
	...
	private _analyzeImports = (
		...
		try {
			...
			// this.unresolved.push(this.localizePath(target));
		} finally {
		...
		for (const local of locals) {
			...
			if (this.isTsFormat(transformedTarget)) {
				...
				this.unrecognized.push(localized);
			}
		}
	};
...
export type CustomLocalPathResolver = (basePath: string, path: string, target: string) => string;
