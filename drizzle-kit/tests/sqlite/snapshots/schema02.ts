...
import { blob, index, integer, primaryKey, real, sqliteTable, text, unique, uniqueIndex } from 'orm044/sqlite-core';
import { v4 } from 'uuid';
...
export const profileAgreements = sqliteTable(
	...
	(table) => [unique('index_4').on(table.profileId)],
);
...
export const selections = sqliteTable(
	...
	(table) => [
		...
		unique('selections_id_profileId_unique').on(table.id, table.profileId),
	],
);
...
export const transactions = sqliteTable(
	...
	(table) => [
		unique('transactions_transactionId_processor_unique').on(
			table.transactionId,
			table.processor,
		),
