...
test.skip('create view #1', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('some_view').as((qb) => qb.select().from(users)),
	};

	const { statements, sqlStatements } = await diffTestSchemasSingleStore(from, to, []);
...
test.skip('create view #2', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('some_view', {}).algorithm('merge').sqlSecurity('definer')
...
test('create view with existing flag', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('some_view', {}).existing(),
	};
...
test('rename view', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('new_some_view', {}).algorithm('merge').sqlSecurity('definer')
			.withCheckOption('cascaded').as(sql`SELECT * FROM ${users}`),
	};
...
test.skip('rename view and alter meta options', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('new_some_view', {}).sqlSecurity('definer')
			.withCheckOption('cascaded').as(sql`SELECT * FROM ${users}`),
	};
...
test.skip('add meta to view', async () => {
	...
	const from = {
		users: users,
		view: singlestoreView('some_view', {}).as(sql`SELECT * FROM ${users}`),
	};
	const to = {
		users: users,
		view: singlestoreView('some_view', {}).algorithm('merge').sqlSecurity('definer')
...
test.skip('alter meta to view', async () => {
	...
	const from = {
		users: users,
		view: singlestoreView('some_view', {}).algorithm('temptable').sqlSecurity('invoker')
			.withCheckOption('cascaded').as(sql`SELECT * FROM ${users}`),
	};
	const to = {
		users: users,
...
test.skip('rename and alter view ".as" value', async () => {
	...
	const to = {
		users: users,
		view: singlestoreView('new_some_view', {}).algorithm('temptable').sqlSecurity('invoker')
			.withCheckOption('cascaded').as(sql`SELECT * FROM ${users} WHERE ${users.id} = 1`),
	};
