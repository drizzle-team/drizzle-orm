...
export const analytics = cockroachSchema('analytics');
export const billing = cockroachSchema('billing');
export const monitoring = cockroachSchema('monitoring');
export const alertAction = cockroachEnum('alert_action', ['email', 'pagerd/ut"\'y', 'slack', 'webhook']);
export const currencyCode = cockroachEnum('currency_code', ['USD', 'EUR', 'GBP', 'UAH', 'JPY']);
export const datasetVisibility = cockroachEnum('dataset_visibility', ['priv"ate', 'team', 'public']);
export const env = cockroachEnum('env', ['dev', 'staging', 'prod']);
export const featureState = cockroachEnum('feature_state', ['enabled', 'disabled', 'gradual']);
export const invoiceStatus = cockroachEnum('invoice_status', ['draft', "iss'ued", 'paid', 'voided', 'failed']);
export const jobState = cockroachEnum('job_state', ['queued', 'running', 'success', 'failed', 'cancelled']);
export const notificationChannel = cockroachEnum('notification_channel', ['email', 'sms', 'in_app', 'webhook']);
export const paymentMethod = cockroachEnum('payment_method', ['card', 'bank_transfer', 'paypal', 'crypto']);
export const pipelineStatus = cockroachEnum('pipeline_status', [
	'created',
	'running',
	'paused',
	'completed',
...
export const roleKind = cockroachEnum('role_kind', ['system', 'custom']);
export const ruleConditionOperator = cockroachEnum('rule_condition_operator', [
	'eq',
	'neq',
	'gt',
	'lt',
...
export const severityLevel = cockroachEnum('severity_level', ['low', 'medium', 'high', 'critical']);
export const userStatus = cockroachEnum('user_status', ['active', 'inactive', 'suspended', 'pending']);
...
export const rolesInCore = core.table('roles', {
	id: bigint({ mode: 'number' }).generatedAlwaysAsIdentity().primaryKey().notNull(),
...
export const apiKeysInCore = core.table('api_keys', {
	...
	index('core_apikey_org_idx').using('btree', table.organizationId.asc()).where(
		sql`(revoked = false)`,
	),
	foreignKey({
		columns: [table.organizationId],
...
export const pipelineRunsInAnalytics = analytics.table('pipeline_runs', {
	...
	index('analytics_pipeline_runs_state_idx').using('btree', table.state.asc()),
	foreignKey({
...
export const objectsInCore = core.table('objects', {
	...
	index('core_objects_bucket_path_gin').using('gin', table.metadata.asc()),
	foreignKey({
...
export const notificationsInCore = core.table('notifications', {
	...
	index('core_notifications_unseen_idx').using('btree', table.userId.asc()).where(
		sql`(seen = false)`,
	),
	foreignKey({
		columns: [table.userId],
...
export const webhooksInCore = core.table('webhooks', {
	...
	index('core_webhooks_org_active_idx').using('btree', table.organizationId.asc()).where(
		sql`(active = true)`,
	),
	foreignKey({
		columns: [table.organizationId],
...
export const ruleConditionsInMonitoring = monitoring.table('rule_conditions', {
	...
	metricKey: text('metric_key').notNull(),
	operator: ruleConditionOperator().notNull().unique('some_name'),
...
export const rateLimitsInCore = core.table('rate_limits', {
	...
	unique('rate_limits_api_key_id_window_start_key').on(table.apiKeyId, table.windowStart),
]);
...
export const invoicesInBilling = billing.table('invoices', {
	...
	index('billing_invoices_status_idx').using('btree', table.status.asc()),
	foreignKey({
...
export const selfRef = core.table('self_ref', {
	id: uuid().defaultRandom().primaryKey().notNull(),
	objectType: text('object_type').notNull().unique().references((): AnyCockroachColumn => selfRef.organizationId),
...
// polices
export const rls = cockroachSchema('rls');
export const documentsInRls = rls.table('documents', {
	...
	cockroachPolicy('documents_delete_own', {
		as: 'permissive',
		for: 'delete',
		to: ['public'],
		using: sql`(owner_id = (CURRENT_USER)::uuid)`,
	}),
	cockroachPolicy('documents_update_own', { as: 'permissive', for: 'update', to: ['public'] }),
	cockroachPolicy('documents_select_own', { as: 'permissive', for: 'select', to: ['public'] }),
]);
...
export const messagesInRls = rls.table.withRLS('messages', {
	...
	cockroachPolicy('messages_delete_own', {
		as: 'permissive',
		for: 'delete',
		to: ['public'],
		using: sql`(sender_id = (CURRENT_USER)::uuid)`,
	}),
	cockroachPolicy('messages_visibility', { as: 'permissive', for: 'select', to: ['public'] }),
]);
...
export const projectsInRls = rls.table('projects', {
	...
	cockroachPolicy('projects_visibility', {
		as: 'permissive',
		for: 'select',
		to: ['public'],
		using: sql`(owner_id = (CURRENT_USER)::uuid)`,
...
export const projectMembersInRls = rls.table.withRLS('project_members', {
	...
	cockroachPolicy('project_members_manage', {
		...
		for: 'all',
		to: ['public'],
		using: sql`(user_id = CURRENT_USER::uuid)`,
	}),
	cockroachPolicy('project_members_visibility', { as: 'permissive', for: 'select', to: ['public'] }),
	check('project_members_role_check', sql`role = ANY (ARRAY['member'::text, 'admin'::text])`),
]);

export const policy = cockroachPolicy('new_policy', {
	as: 'restrictive',
	to: 'root',
	withCheck: sql`1 = 1`,
	for: 'all',
