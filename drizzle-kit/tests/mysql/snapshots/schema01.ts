...
export const allDataTypes = mysqlTable('all_data_types', {
	...
	bigint63: bigint('bigint_63', { mode: 'bigint' }).default(9223372036854775807n),
	real: real('real').default(10.123),
	realPrecisionScale: real('real_precision_scale', { precision: 6, scale: 2 }).default(10.123),
	...
	floatPrecisionScale: float('float_precision_scale', { precision: 6, scale: 2 }).default(10.123),
	floatUnsigned: float('float', { unsigned: true }).default(10.123),
	...
	blobExpression: blob('blob_expression').default(sql`('hello' + ' world')`),
	blobString: blob('blob_string', { mode: 'string' }).default(`text'"\`:[]{},text`),
	blobBuffer: blob('blob_buffer', { mode: 'buffer' }).default(Buffer.from(`text'"\`:[]{},text`)),
	tinyblobString: tinyblob('tinyblob_string', { mode: 'string' }).default(`text'"\`:[]{},text`),
	tinyblobBuffer: tinyblob('tinyblob_buffer', { mode: 'buffer' }).default(Buffer.from(`text'"\`:[]{},text`)),
	mediumblobString: mediumblob('mediumblob_string', { mode: 'string' }).default(`text'"\`:[]{},text`),
	mediumblobBuffer: mediumblob('mediumblob_buffer', { mode: 'buffer' }).default(Buffer.from(`text'"\`:[]{},text`)),
	longblobString: longblob('longblob_string', { mode: 'string' }).default(`text'"\`:[]{},text`),
	longblobBuffer: longblob('longblob_buffer', { mode: 'buffer' }).default(Buffer.from(`text'"\`:[]{},text`)),
	char0: char('char0').default(`text'"\`:[]{},text`),
	char: char('char', { length: 255 }).default(`text'"\`:[]{},text`),
	varchar0: varchar('varchar0').default(`text'"\`:[]{},text`),
	varchar: varchar('varchar', { length: 256 }).default(`text'"\`:[]{},text`),
	text: text('text').default(`text'"\`:[]{},text`),
	tinytext: tinytext('tinytext').default(`text'"\`:[]{},text`),
	mediumtext: mediumtext('mediumtext').default(`text'"\`:[]{},text`),
	longtext: longtext('longtext').default(`text'"\`:[]{},text`),
	boolean: boolean('boolean').default(true),
	booleanNull: boolean('boolean_null').default(sql`null`),
	date: date('date', { mode: 'date' }),
	datetime: datetime('datetime', { mode: 'date' }).default(new Date('2025-05-23T12:53:53.115Z')),
	datetimeFsp: datetime('datetime_fsp', { mode: 'date', fsp: 3 }).default(new Date('2025-05-23T12:53:53.115Z')),
	time: time('time').default('15:50:33.123'),
	timeFsp: time('time_fsp', { fsp: 3 }).default('15:50:33.123'),
	year: year('year').default(2025),
	timestamp: timestamp('timestamp', { mode: 'date' }).default(new Date('2025-05-23T12:53:53.115Z')),
	...
	jsonArray: json('json_array').default([9223372036854775807n, 9223372036854775806n]),
	json: json('json').default({ key: `text'"\`:[]{},text` }),
	mysqlEnum: mysqlEnum('popularity', ['unknown', 'known', 'popular', `text'"\`:[]{},text`]).default(
		`text'"\`:[]{},text`,
	),
});
...
export const table1 = mysqlTable('table1', {
	...
	column2: varchar({ length: 256 }).generatedAlwaysAs((): SQL => sql`${table1.column1} || 'hello'`, { mode: 'stored' }),
	column3: varchar({ length: 256 }).generatedAlwaysAs((): SQL => sql`${table1.column1} || 'hello'`, {
		mode: 'virtual',
	}),
