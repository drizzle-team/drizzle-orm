...
import {
  ...
  pgPushIntrospect,
} from "src/cli/commands/pull-postgres";
...
export const drizzleToDDL = (
  ...
  const tables = Object.values(schema).filter((it) =>
    is(it, PgTable)
  ) as PgTable[];
  const schemas = Object.values(schema).filter((it) =>
    is(it, PgSchema)
  ) as PgSchema[];
  const enums = Object.values(schema).filter((it) =>
    isPgEnum(it)
  ) as PgEnum<any>[];
  const sequences = Object.values(schema).filter((it) =>
    isPgSequence(it)
  ) as PgSequence[];
  const roles = Object.values(schema).filter((it) =>
    is(it, PgRole)
  ) as PgRole[];
  const policies = Object.values(schema).filter((it) =>
    is(it, PgPolicy)
  ) as PgPolicy[];
...
export const diffTestSchemas = async (
  ...
  const { ddl: ddl1, errors: err1 } = drizzleToDDL(left);
  ...
  if (!cli) {
    const { sqlStatements, statements, groupedStatements, errors } =
      await ddlDiff(
        ddl1,
        ddl2,
...
export const diffTestSchemasPush = async (
  ...
  const shouldRunApply =
    sqlStatementsToRun.runApply === undefined
      ? true
  ...
  for (const view of materializedViewsForRefresh) {
    ...
    await client.exec(
      `REFRESH MATERIALIZED VIEW "${viewConf.schema ?? "public"}"."${
        viewConf.name
  ...
  const leftRoles = Object.values(right).filter((it) =>
    is(it, PgRole)
  ) as PgRole[];
  const leftPolicies = Object.values(right).filter((it) =>
    is(it, PgPolicy)
  ) as PgPolicy[];
  const leftViews = Object.values(right).filter((it) =>
    isPgView(it)
  ) as PgView[];
  const leftMaterializedViews = Object.values(right).filter((it) =>
    isPgMaterializedView(it)
  ) as PgMaterializedView[];
  ...
  if (!cli) {
    ...
    const { hints, statements: nextStatements } = await suggestions(
      db,
      statements
    );
...
export const applyPgDiffs = async (
  ...
  const policies = Object.values(sn).filter((it) =>
    is(it, PgPolicy)
  ) as PgPolicy[];
  const materializedViews = Object.values(sn).filter((it) =>
    isPgMaterializedView(it)
  ) as PgMaterializedView[];
