...
import { diffTestSchemas, diffTestSchemasPush } from 'tests/postgres/mocks';
...
const pgSuite: DialectSuite = {
	...
	async addNotNull() {
		...
		const { statements } = await diffTestSchemasPush(client, schema1, schema2, [], false, ['public']);
		const query = async (sql: string, params?: any[]) => {
			const result = await client.query(sql, params ?? []);
			return result.rows as any[];
		};

		const { statements: st, hints } = await suggestions({ query }, statements);

		expect(st).toStrictEqual(['ALTER TABLE "User" ALTER COLUMN "email" SET NOT NULL;']);
	},
	...
	async addNotNullWithDataNoRollback() {
		...
		const { hints, statements: to } = await suggestions({ query }, statements);

		expect(hints).toStrictEqual([]);
		expect(to).toStrictEqual(['ALTER TABLE "User" ALTER COLUMN "email" SET NOT NULL;']);
	},
