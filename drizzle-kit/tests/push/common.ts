...
export const run = (
  ...
  test("Indexes properties that should not trigger push changes", () =>
    suite.indexesToBeNotTriggered(context));
  ...
  test("Add not null to a column with null data. Should rollback", () =>
    suite.addNotNullWithDataNoRollback());

  test("Add basic sequences", () => suite.addBasicSequences());

  test("Add generated column", () => suite.addGeneratedColumn(context));
  test("Add generated constraint to an existing column", () =>
    suite.addGeneratedToColumn(context));
  test("Drop generated constraint from a column", () =>
    suite.dropGeneratedConstraint(context));
  // should ignore on push
  test("Alter generated constraint", () =>
    suite.alterGeneratedConstraint(context));
  test("Create table with generated column", () =>
    suite.createTableWithGeneratedConstraint(context));
