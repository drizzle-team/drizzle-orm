...
import type { PostgresHttpDatabase } from 'drizzle-orm/postgres/http';
...
describe('migrator', () => {
	...
	test('migrator : a failing migration leaves nothing applied', async ({ db }) => {
		await db.execute(sql`drop schema if exists "drizzle" cascade;`);
		await db.execute(sql`drop table if exists neon_http_mig_ok`);

		const migrationDir = './migrations/postgres-neon-http-fail';
		if (existsSync(migrationDir)) rmSync(migrationDir, { recursive: true });
		mkdirSync(`${migrationDir}/20240101010101_ok`, { recursive: true });
		writeFileSync(`${migrationDir}/20240101010101_ok/migration.sql`, 'CREATE TABLE "neon_http_mig_ok" ("id" integer);');
		mkdirSync(`${migrationDir}/20240202020202_bad`, { recursive: true });
		writeFileSync(
			`${migrationDir}/20240202020202_bad/migration.sql`,
			'CREATE TABLE "neon_http_mig_ok" ("id" integer);',
		);
		...
		const present = await db.execute<{ present: boolean }>(
			sql`select to_regclass('public.neon_http_mig_ok') is not null as present`,
		);
...
describe('driver-specific', () => {
	...
	// Can't test shapes via generic codecs test
