...
export function tests() {
	describe('common', () => {
		...
		test('insert multiple rows into table with generated identity column', async (ctx) => {
			...
			await db.execute(sql`drop table if exists ${users}`);
			await db.execute(
				sql`create table ${users} ("id" integer generated always as identity, "id1" integer generated by default as identity, "name" text)`,
			);
			...
			result = await db.insert(users).overridingSystemValue().values([
