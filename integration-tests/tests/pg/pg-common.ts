...
export function tests() {
  describe("common", () => {
    ...
    test("build query insert with onConflict do update / targetWhere", async (ctx) => {
      const { db } = ctx.pg;
      ...
      const query = db
        ...
        .onConflictDoUpdate({
          ...
          set: { name: "John1" },
          targetWhere: sql`${usersTable.name} = 'John'`,
        })
        .toSQL();

      expect(query).toEqual({
        sql: 'insert into "users" ("id", "name", "verified", "jsonb", "created_at") values (default, $1, default, $2, default) on conflict ("id") where "users"."name" = \'John\' do update set "name" = $3',
        params: ["John", '["foo","bar"]', "John1"],
      });
    });

    test("build query insert with onConflict do update / setWhere", async (ctx) => {
      const { db } = ctx.pg;
      ...
      const query = db
        ...
        .onConflictDoUpdate({
          target: usersTable.id,
          set: { name: "John1" },
          setWhere: sql`${usersTable.name} <> 'John1'`,
        })
        .toSQL();

      expect(query).toEqual({
        sql: 'insert into "users" ("id", "name", "verified", "jsonb", "created_at") values (default, $1, default, $2, default) on conflict ("id") do update set "name" = $3 where "users"."name" <> \'John1\'',
        params: ["John", '["foo","bar"]', "John1"],
      });
    });

    test("insert with onConflict do update / targetWhere respects partial unique index", async (ctx) => {
      const { db } = ctx.pg;

      const cities = pgTable("cities_target_where", {
        id: serial("id").primaryKey(),
        name: text("name").notNull(),
      ...
      await db.execute(sql`drop table if exists ${cities}`);
      await db.execute(
        sql`create table ${cities} (id serial primary key, name text not null, population integer default 0)`,
      );
      await db.execute(
        sql`create unique index cities_name_big_idx on ${cities} (name) where (population > 1000)`,
      );

      await db
        .insert(cities)
        .values({ id: 1, name: "Lagos", population: 5000 });
      // Does NOT hit the partial index (population <= 1000), so this is a plain insert, not a conflict.
      await db.insert(cities).values({ id: 2, name: "Lagos", population: 100 });

      const res = await db.select().from(cities).orderBy(cities.id);
      expect(res).toEqual([
        { id: 1, name: "Lagos", population: 5000 },
        { id: 2, name: "Lagos", population: 100 },
      ]);

      await db.execute(sql`drop table ${cities}`);
    });

    test("insert with onConflict do update / setWhere prevents no-op update", async (ctx) => {
      const { db } = ctx.pg;
      ...
      await db
        ...
        .onConflictDoUpdate({
          target: usersTable.id,
          set: { name: "John" },
          setWhere: sql`excluded.name <> ${usersTable.name}`,
        });
    ...
    test("onConflictDoUpdate throws when combining deprecated where with targetWhere/setWhere", async (ctx) => {
      const { db } = ctx.pg;
    ...
    test("having", async (ctx) => {
      ...
      const result = await db
        .select({
          ...
          usersCount: sql<number>`count(${users2Table.id})::int`.as(
            "users_count",
          ),
        })
        .from(citiesTable)
    ...
    test("select from enum", async (ctx) => {
      ...
      const forceEnum = pgEnum("force", [
        "isometric",
        "isotonic",
        "isokinetic",
      ]);

      const levelEnum = pgEnum("level", [
        "beginner",
        "intermediate",
        "advanced",
      ]);
    ...
    test("select from sql", async (ctx) => {
      ...
      const intervals = db.$with("intervals").as(
        db
          .select({
            startTime:
              sql<string>`(date'2023-03-01'+ x * '1 day'::interval)`.as(
                "start_time",
              ),
            endTime:
              sql<string>`(date'2023-03-01'+ (x+1) *'1 day'::interval)`.as(
                "end_time",
              ),
          })
    ...
    test("sql.identifier escape", async (ctx) => {
      ...
      const userInput =
        'id" ASC, CAST((SELECT name FROM users LIMIT 1) AS int)--';
