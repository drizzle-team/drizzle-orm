...
it.layer(TestLive)((it) => {
	...
	it.effect('all types ~codecs~', () =>
		Effect.gen(function*() {
			const allTypesTable = pgTable('all_types', (t) => ({
			...
			yield* db.execute('CREATE EXTENSION IF NOT EXISTS postgis;');
			yield* db.execute('CREATE EXTENSION IF NOT EXISTS vector;');
			yield* db.execute('DROP TABLE IF EXISTS all_types CASCADE;');
			yield* push({ allTypesTable });
