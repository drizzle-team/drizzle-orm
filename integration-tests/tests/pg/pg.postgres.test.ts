...
declare module 'vitest' {
	export interface TestContext {
		postgresDbV2: PostgresDatabase<typeof relations>;
		postgresClient: Pool;
	}
}
...
beforeAll(async () => {
	...
	do {
		try {
			db = drizzle(connectionString, { relations, logger: ENABLE_LOGGING });
...
beforeEach(async (ctx) => {
	ctx.postgresDbV2 = db;
	ctx.postgresClient = db.$client;

	await ctx.postgresDbV2.execute(sql`drop schema public cascade`);
	await ctx.postgresDbV2.execute(sql`drop schema if exists rqb_test_schema cascade`);
	await ctx.postgresDbV2.execute(sql`create schema public`);
	await ctx.postgresDbV2.execute(sql`create schema rqb_test_schema`);
	await ctx.postgresDbV2.execute(
		sql`
			CREATE TABLE "users" (
