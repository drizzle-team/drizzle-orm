...
const resetDeepNullification = (db: PgEffectDatabase<any, any, any>) =>
	Effect.gen(function*() {
		yield* db.execute(sql`DROP VIEW IF EXISTS ${dnStaffView}`);
		for (const t of Object.values(dnTables)) yield* db.execute(sql`DROP TABLE IF EXISTS ${t} CASCADE`);
	});

const runDeepNullification = (db: PgEffectDatabase<any, any, any>) =>
	Effect.gen(function*() {
	...
	it.layer(testLayer)('common', (it) => {
		...
		it.effect('Default value priority', () =>
			Effect.gen(function*() {
				...
				yield* db.execute(sql`DROP TABLE IF EXISTS ${exTbl} CASCADE`);
				yield* push(db, { exTbl });
