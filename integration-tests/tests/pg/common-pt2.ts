...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.concurrent('all types', async ({ db, push }) => {
			const { en, allTypesTable } = makeAllTypes('all_types_48', 'en_48');
		...
		test.skipIf(Date.now() < +new Date('2026-09-29')).concurrent('insert into ... select #2', async ({ db, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-09-29'))('insert with inline params in sql', async ({ db }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-09-29'))(
		...
		test.skipIf(Date.now() < +new Date('2026-09-29'))('db/js timestamp comparison', async ({ db, push }) => {
			const table1 = pgTable('table1', {
				id: integer(),
