...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.skipIf(Date.now() < +new Date('2026-02-25')).concurrent('view #1', async ({ push, createDB }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-02-25')).concurrent(
		...
		test.skipIf(Date.now() < +new Date('2026-02-25')).concurrent('insert into ... select #2', async ({ db, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-02-25'))('casing in sql``', async ({ createDB, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-02-25'))('db/js timestamp comparison', async ({ db, push }) => {
			const table1 = pgTable('table1', {
				id: integer(),
