...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.skipIf(Date.now() < +new Date('2026-04-20')).concurrent('view #1', async ({ push, createDB }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-04-20')).concurrent(
		...
		test.skipIf(Date.now() < +new Date('2026-04-20')).concurrent('insert into ... select #2', async ({ db, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-04-20'))(
		...
		test.skipIf(Date.now() < +new Date('2026-04-20'))('casing in sql``', async ({ createDB, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-04-20'))('db/js timestamp comparison', async ({ db, push }) => {
			const table1 = pgTable('table1', {
				id: integer(),
