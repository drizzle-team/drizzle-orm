...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.skipIf(Date.now() < +new Date('2026-09-17')).concurrent(
		...
		test.skipIf(Date.now() < +new Date('2026-09-17')).concurrent('insert into ... select #2', async ({ db, push }) => {
		...
		test.skipIf(Date.now() < +new Date('2026-09-17'))(
		...
		test.skipIf(Date.now() < +new Date('2026-09-17'))('db/js timestamp comparison', async ({ db, push }) => {
			const table1 = pgTable('table1', {
				id: integer(),
