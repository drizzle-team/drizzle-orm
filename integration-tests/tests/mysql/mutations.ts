...
import { orders, usersMigratorTable, usersOnUpdate, usersTable } from './schema2';

export function tests(vendor: 'mysql' | 'planetscale', test: Test, exclude: Set<string> = new Set<string>([])) {
	test.beforeEach(async ({ client, task, skip }) => {
		if (exclude.has(task.name)) skip();
		const { batch } = client;

		await batch([
			`drop table if exists userstest, users2, cities, all_types;`,
		]);
		await batch([
	...
	describe('mutations', () => {
		test('insert+update+delete returning sql', async ({ db }) => {
			const [result, _] = await db.insert(usersTable).values({ name: 'John' });
			const res1 = await db.update(usersTable).set({ name: 'Jane' }).where(eq(usersTable.name, 'John'));
			const res2 = await db.delete(usersTable).where(eq(usersTable.name, 'Jane'));
		...
		test('update with returning all fields + partial', async ({ db }) => {
	...
	test('Mysql enum as ts enum', async ({ db }) => {
		enum Test {
			a = 'a',
			b = 'b',
			c = 'c',
	...
	test('update with limit and order by', async ({ db }) => {
		await db.insert(usersTable).values([
	...
	test('delete with limit and order by', async ({ db }) => {
		await db.insert(usersTable).values([
			{ name: 'Barry', verified: false },
			{ name: 'Alan', verified: false },
			{ name: 'Carl', verified: false },
