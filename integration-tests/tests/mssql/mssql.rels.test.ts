...
import { desc, DrizzleError, eq, exists, gt, gte, or, sql, TransactionRollbackError } from 'drizzle-orm';
...
test.beforeEach(async ({ db }) => {
	await db.execute(sql`drop view if exists [rqb_users_view]`);
	await db.execute(sql`drop view if exists [rqb_test_schema].[users_sch_view]`);
	await db.execute(sql`drop table if exists [rqb_test_schema].[users_to_groups]`);
	await db.execute(sql`drop table if exists [rqb_test_schema].[posts]`);
	await db.execute(sql`drop table if exists [rqb_test_schema].[groups]`);
	await db.execute(sql`drop table if exists [rqb_test_schema].[users]`);
	await db.execute(sql`drop table if exists [student_grades]`);
	await db.execute(sql`drop table if exists [course_offerings]`);
	...
	await db.execute(
		sql`if not exists (select 1 from sys.schemas where name = 'rqb_test_schema') exec('create schema [rqb_test_schema]')`,
	);
	...
	await db.execute(
		sql`
			CREATE TABLE [comment_likes] (
				[id] int identity PRIMARY KEY NOT NULL,
				[creator] int null foreign key REFERENCES [users]([id]),
				[comment_id] int null foreign key REFERENCES [comments]([id]),
				[created_at] datetime DEFAULT CURRENT_TIMESTAMP NOT NULL
			);
		`,
	);
	...
	await db.execute(
		sql`
			CREATE TABLE [rqb_test_schema].[users] (
				...
				[name] varchar(100) NOT NULL,
				[verified] bit DEFAULT 0 NOT NULL,
				[invited_by] int null foreign key REFERENCES [rqb_test_schema].[users]([id])
			);
		`,
	);
	await db.execute(
	...
	await db.execute(
		sql`
			CREATE TABLE [rqb_test_schema].[posts] (
				[id] int identity PRIMARY KEY NOT NULL,
				[content] varchar(100) NOT NULL,
				[owner_id] int null foreign key REFERENCES [rqb_test_schema].[users]([id]),
				[created_at] datetime DEFAULT CURRENT_TIMESTAMP NOT NULL
			);
		`,
	);
	await db.execute(
		sql`
			CREATE TABLE [rqb_test_schema].[users_to_groups] (
				[id] int identity PRIMARY KEY NOT NULL,
				[user_id] int foreign key REFERENCES [rqb_test_schema].[users]([id]),
				[group_id] int foreign key REFERENCES [rqb_test_schema].[groups]([id])
			);
		`,
	);
	...
	await db.execute(
		sql`
			CREATE VIEW [rqb_users_view] AS (select [users].[id], [users].[name], [users].[verified], [users].[invited_by], [posts].[content], [posts].[created_at], (select count(*) from [users] as [count_source] where [users].[id] <> 2) as [count] from [users] left join [posts] on [users].[id] = [posts].[owner_id]);
		`,
	);
	await db.execute(
		sql`
...
test('[Find Many] Get users with posts + prepared + limit + offset + where', async ({ db }) => {
	...
	const prepared = db.query.usersTable.findMany({
		...
		offset: sql.placeholder('uOffset'),
		orderBy: { id: 'asc' },
		where: { OR: [{ id: sql.placeholder('id') }, { id: 3 }] },
		with: {
			posts: {
				where: { id: sql.placeholder('pid') },
				limit: sql.placeholder('pLimit'),
			},
		},
	}).prepare();
...
test('[Find Many] offset without orderBy is rejected', async ({ db }) => {
	expect(() => db.query.usersTable.findMany({ offset: 1 }).toSQL())
		.toThrowError(DrizzleError);
	expect(() => db.query.usersTable.findMany({ offset: 1 }).toSQL())
		.toThrowError(/uses "offset" without "orderBy"/);

	expect(() =>
		db.query.usersTable.findMany({
			with: { posts: { offset: 1 } },
		}).toSQL()
	).toThrowError(/\("posts"\) uses "offset" without "orderBy"/);

	const limitOnly = db.query.usersTable.findMany({ limit: 1 }).toSQL();
	expect(limitOnly.sql).toMatch(/top\(/);

	const ordered = db.query.usersTable.findMany({ limit: 2, offset: 1, orderBy: { id: 'asc' } }).toSQL();
	expect(ordered.sql).toMatch(/order by .* offset .* rows fetch next .* rows only/);
	expect(ordered.sql).not.toMatch(/select null/);
});
...
test('Get user with invitee and custom fields', async ({ db }) => {
	...
	const usersWithInvitee = await db.query.usersTable.findMany({
		extras: { lower: ({ name }) => sql<string>`lower(${name})`.as('lower_name') },
		with: {
			invitee: {
				extras: { lower: ({ name }) => sql<string>`lower(${name})`.as('lower_name') },
			},
...
test('Get user with invitee and posts + limits + custom fields in each', async ({ db }) => {
	...
	const response = await db.query.usersTable.findMany({
		...
		with: {
			invitee: {
				extras: { lower: ({ name }) => sql<string>`lower(${name})`.as('lower_invitee_name') },
			},
			posts: {
				limit: 1,
				extras: { lower: ({ content }) => sql<string>`lower(${content})`.as('lower_content') },
			},
		},
	});
...
test('Get user with invitee and posts + custom fields in each', async ({ db }) => {
	...
	const response = await db.query.usersTable.findMany({
		...
		with: {
			...
			posts: {
				extras: { lower: ({ content }) => sql<string>`lower(${content})`.as('lower_name') },
			},
		},
	});
...
test('Correct error message on unknown column', async ({ db }) => {
...
test('Correct error message on unknown relation', async ({ db }) => {
	const query = db.query.usersTable.findFirst({
		with: {
			posts: true,
			// @ts-expect-error
...
test('Disallow unknown keys in filters', async ({ db }) => {
	const query = db.query.usersTable.findFirst({
		where: {
			name: 'NAME',
			// @ts-expect-error
...
test('Force optional on where on non-optional relation query', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex', invitedBy: 1 },
...
test('Get user with posts and posts with comments and comments with owner where exists', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get custom schema groups with filtered users + where', async ({ db }) => {
...
test('[Find Many .through] Get custom schema users with filtered groups + where', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with filtered users', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with filtered users + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with filtered view users + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with first user', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + custom', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + limit + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + orderBy', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + orderBy + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with users + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get schema groups with filtered view users + where', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get schema view users with filtered groups + where', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by existing groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by groups with groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by groups with groups filtered by users', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by nonexisting groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users filtered by users of groups with groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with existing groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with filtered groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with filtered groups + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with first group', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + custom', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + limit + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + orderBy', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + orderBy + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with groups + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get users with nonexisting groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get view users with filtered groups + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Through with uneven relation column count', async ({ db }) => {
...
test('[Find Many .through] Through with uneven relation column count - reverse', async ({ db }) => {
	await db.insert(students).values([{
		name: 'First',
	}, {
		name: 'Second',
...
test('[Find Many] Get custom schema posts with filtered authors + where', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
	...
	await db.insert(schemaPosts).values([
		{ ownerId: 1, content: 'Message1.1' },
		{ ownerId: 2, content: 'Message2.1' },
		{ ownerId: 3, content: 'Message3.1' },
		{ ownerId: 1, content: 'Message1.2' },
		{ ownerId: 2, content: 'Message2.2' },
		{ ownerId: 3, content: 'Message3.2' },
		{ ownerId: 1, content: 'Message1.3' },
		{ ownerId: 2, content: 'Message2.3' },
		{ ownerId: 3, content: 'Message3.3' },
		{ ownerId: 1, content: 'Post1.1' },
		{ ownerId: 2, content: 'Post2.1' },
		{ ownerId: 3, content: 'Post3.1' },
		{ ownerId: 1, content: 'Post1.2' },
		{ ownerId: 2, content: 'Post2.2' },
		{ ownerId: 3, content: 'Post3.2' },
		{ ownerId: 1, content: 'Post1.3' },
		{ ownerId: 2, content: 'Post2.3' },
		{ ownerId: 3, content: 'Post3.3' },
	]);
...
test('[Find Many] Get custom schema users with filtered posts + where', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with filtered authors', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with filtered authors + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with view users + filter by SQL.Aliased with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with view users + filter by joined column with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with view users + filter with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with view users with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema posts with view users + filter by SQL.Aliased with posts', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema posts with view users + filter by joined column with posts', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema posts with view users + filter with posts', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema posts with view users with posts', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema view users with posts', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema view users with posts + filter by SQL.Aliased field', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get schema view users with posts + filter by joined field', async ({ db }) => {
	await db.insert(schemaUsers).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users + filter users by posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by existing posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by nonexisting posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by own columns and posts with filtered posts by own columns and author', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by posts with AND', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by posts with NOT', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users filtered by posts with OR', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users with filtered posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users with filtered posts + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users with posts + filter posts by author', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users with posts + filter users by existing posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get users with posts + filter users by posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get view users with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get view users with posts + filter by SQL.Aliased field', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get view users with posts + filter by joined field', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Shortcut form placeholders in filters - column not', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Shortcut form placeholders in filters - column or', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Shortcut form placeholders in filters - eq', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Shortcut form placeholders in filters - or', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get groups with users', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get groups with users + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get groups with users + limit + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get groups with users + orderBy', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get groups with users + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups + limit + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups + orderBy', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups + orderBy + limit', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find One .through] Get users with groups + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('alltypes', async ({ db }) => {
	await db.execute(sql`drop table if exists [all_types_rqb]`);
	await db.execute(sql`
		create table [all_types_rqb] (
			[id] int primary key not null,
			[c_int] int, [c_bigint] bigint, [c_bigint_num] bigint,
			[c_decimal] decimal(38,9), [c_numeric] numeric(38,9),
			[c_float] float, [c_real] real, [c_bit] bit, [c_varchar] varchar(50),
			[c_date] date, [c_date_s] date,
			[c_datetime] datetime, [c_datetime_s] datetime,
			[c_datetime2] datetime2(3), [c_datetime2_s] datetime2(3),
			[c_dto] datetimeoffset(3), [c_dto_s] datetimeoffset(3),
			[c_time] time, [c_time_s] time,
			[c_binary] binary(5), [c_varbinary] varbinary(50),
			[c_custom_bigint] bigint, [c_custom_bigint_codec] bigint
		)
	`);

	try {
		await db.insert(usersTable).values({ id: 1, name: 'First' });

		await db.insert(allTypesTable).values({
			id: 1,
			cInt: 621,
			cBigint: 5044565289845416380n,
			cBigintNum: 9007199254740991,
			cDecimal: '123456789.987654321',
			cNumeric: '987654321.123456789',
			cFloat: 15.35325689124218,
			cReal: 1.5,
			cBit: true,
			cVarchar: 'C4-',
			cDate: new Date('2025-03-12T00:00:00Z'),
			cDateStr: '2025-03-12',
			cDatetime: new Date('2025-03-12T01:32:41.000Z'),
			cDatetimeStr: '2025-03-12T01:32:41.000Z',
			cDatetime2: new Date('2025-03-12T01:32:41.623Z'),
			cDatetime2Str: '2025-03-12T01:32:41.623Z',
			cDto: new Date('2025-03-12T01:32:41.623Z'),
			cDtoStr: '2025-03-12T01:32:41.623Z',
			cTime: new Date('1970-01-01T13:59:28.000Z'),
			cTimeStr: '13:59:28',
			cBinary: Buffer.from('BYTES'),
			cVarbinary: Buffer.from('BYTES'),
			cCustomBigint: 5044565289845416380n,
			cCustomBigintCodec: 5044565289845416380n,
		});
		...
		type ExpectedType = {
			...
			cDatetime: Date | null;
			cDatetimeStr: string | null;
			cDatetime2: Date | null;
			cDatetime2Str: string | null;
			cDto: Date | null;
			cDtoStr: string | null;
			cTime: Date | null;
			cTimeStr: string | null;
			cBinary: Buffer | null;
			cVarbinary: Buffer | null;
			cCustomBigint: bigint | null;
			cCustomBigintCodec: bigint | null;
		};
		...
		const expectedRes: ExpectedType[] = [
			{
				...
				cBit: true,
				cVarchar: 'C4-',
				cDate: new Date('2025-03-12T00:00:00.000Z'),
				cDateStr: '2025-03-12',
				cDatetime: new Date('2025-03-12T01:32:41.000Z'),
				cDatetimeStr: '2025-03-12T01:32:41.000Z',
				cDatetime2: new Date('2025-03-12T01:32:41.623Z'),
...
test('[Find Many] Get subquery users with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get subquery users with posts + filter by SQL.Aliased field', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get subquery users with posts + filter by joined field', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with subquery users with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get posts with subquery users + filter with posts', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get subquery users with filtered groups + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many .through] Get groups with filtered subquery users + where', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
...
test('[Find Many] Get subquery users with posts with subquery authors', async ({ db }) => {
	await db.insert(usersTable).values([
		{ id: 1, name: 'Dan' },
		{ id: 2, name: 'Andrew' },
		{ id: 3, name: 'Alex' },
