...
export function tests(driver?: string) {
	describe('common', () => {
		...
		test('test $onUpdateFn and $onUpdate works updating', async (ctx) => {
			...
			const { updatedAt, ...rest } = getTableColumns(usersOnUpdate);
			const initials = await db.select({ id: usersOnUpdate.id, updatedAt: usersOnUpdate.updatedAt }).from(usersOnUpdate);
			...
			expect(
				...
				{ id: 1, name: 'Angel', updateCounter: 2, alwaysNull: null },
				{ id: 2, name: 'Jane', updateCounter: 1, alwaysNull: null },
				{ id: 3, name: 'Jack', updateCounter: 1, alwaysNull: null },
				{ id: 4, name: 'Jill', updateCounter: 1, alwaysNull: null },
			]);
