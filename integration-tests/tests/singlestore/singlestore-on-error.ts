...
/**
 * Integration tests for the `onError` callback in DrizzleConfig (SingleStore).
 *
 * Setup required in the main test file (e.g. singlestore.test.ts):
 *
 ...
 *
 *   beforeEach((ctx) => {
 *     ctx.onErrorSingleStore = { db: onErrorDb, onError: onErrorFn };
 *   });
 *
 *   onErrorTests();
 */
export function tests() {
	describe('onError callback', () => {
		beforeEach((ctx) => {
			ctx.onErrorSingleStore.onError.mockClear();
		});

		test('onError is called when a query fails', async (ctx) => {
			const { db, onError } = ctx.onErrorSingleStore;
