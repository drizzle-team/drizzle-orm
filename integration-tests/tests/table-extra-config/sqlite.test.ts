...
import { getTableConfig, integer, primaryKey, sqliteTable, uniqueIndex } from 'drizzle-orm/sqlite-core';
...
const users = sqliteTable('users', { id: integer('id') }, (t) => [
	uniqueIndex('users_id_idx').on(t.id),
	primaryKey({ columns: [t.id] }),
]);

// @ts-expect-error
const wrapped = sqliteTable('users', { id: integer('id') }, (t) => [{ idIdx: uniqueIndex('users_idx').on(t.id) }]);

// @ts-expect-error
const number = sqliteTable('users', { id: integer('id') }, () => [42]);

// @ts-expect-error
const mimic = sqliteTable('users', { id: integer('id') }, () => [{ name: 'users_idx', columns: ['id'] }]);

// @ts-expect-error
const foreignClass = sqliteTable('users', { id: integer('id') }, () => [new Unrelated()]);

// @ts-expect-error
const foreignDialect = sqliteTable('users', { id: integer('id') }, () => [pgIndex]);

describe('sqlite extra config', () => {
	test('Expected form', () => {
