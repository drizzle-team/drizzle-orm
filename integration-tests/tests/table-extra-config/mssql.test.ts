import { getTableConfig, int, mssqlTable, primaryKey, uniqueIndex } from 'drizzle-orm/mssql-core';
...
const users = mssqlTable('users', { id: int('id') }, (t) => [
	uniqueIndex('users_id_idx').on(t.id),
	primaryKey({ columns: [t.id] }),
]);

// @ts-expect-error
const wrapped = mssqlTable('users', { id: int('id') }, (t) => [{ idIdx: uniqueIndex('users_idx').on(t.id) }]);

// @ts-expect-error
const number = mssqlTable('users', { id: int('id') }, () => [42]);

// @ts-expect-error
const mimic = mssqlTable('users', { id: int('id') }, () => [{ name: 'users_idx', columns: ['id'] }]);

// @ts-expect-error
const foreignClass = mssqlTable('users', { id: int('id') }, () => [new Unrelated()]);

// @ts-expect-error
const foreignDialect = mssqlTable('users', { id: int('id') }, () => [pgIndex]);

describe('mssql extra config', () => {
	test('Expected form', () => {
