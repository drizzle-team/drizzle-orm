import { getTableConfig, int, mysqlTable, primaryKey, uniqueIndex } from 'drizzle-orm/mysql-core';
...
const users = mysqlTable('users', { id: int('id') }, (t) => [
	uniqueIndex('users_id_idx').on(t.id),
	primaryKey({ columns: [t.id] }),
]);

// @ts-expect-error
const wrapped = mysqlTable('users', { id: int('id') }, (t) => [{ idIdx: uniqueIndex('users_idx').on(t.id) }]);

// @ts-expect-error
const number = mysqlTable('users', { id: int('id') }, () => [42]);

// @ts-expect-error
const mimic = mysqlTable('users', { id: int('id') }, () => [{ name: 'users_idx', columns: ['id'] }]);

// @ts-expect-error
const foreignClass = mysqlTable('users', { id: int('id') }, () => [new Unrelated()]);

// @ts-expect-error
const foreignDialect = mysqlTable('users', { id: int('id') }, () => [pgIndex]);

describe('mysql extra config', () => {
	test('Expected form', () => {
