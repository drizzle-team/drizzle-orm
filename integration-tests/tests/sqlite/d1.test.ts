...
test('migrator', async ({ db }) => {
	...
	await migrate(db as DrizzleD1Database<typeof relations>, { migrationsFolder: './drizzle2/sqlite' });
...
test('migrator : migrate with custom table', async ({ db }) => {
	...
	await migrate(db as DrizzleD1Database<typeof relations>, {
...
test('migrator : --init', async ({ db }) => {
	...
	const migratorRes = await migrate(db as DrizzleD1Database<typeof relations>, {
		migrationsFolder: './drizzle2/sqlite',
...
test('migrator: local migration is unapplied. Migrations timestamp is less than last db migration', async ({ db }) => {
	...
	await migrate(db as DrizzleD1Database<typeof relations>, { migrationsFolder: migrationDir });
