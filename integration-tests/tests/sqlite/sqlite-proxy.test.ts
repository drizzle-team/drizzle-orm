...
test('migrator', async ({ db, serverSimulator }) => {
	...
	await migrate(db as SqliteRemoteDatabase<any, typeof relations>, async (queries) => {
		try {
...
test('migrator : --init', async ({ db, serverSimulator }) => {
	...
	const migratorRes = await migrate(db as SqliteRemoteDatabase<any, typeof relations>, async (queries) => {
		try {
			serverSimulator.migrations(queries);
		} catch (e) {
			console.error(e);
