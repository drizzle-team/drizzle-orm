...
beforeAll(() => {
	...
	db = drizzle({ client: new Database(dbPath), schema, logger: ENABLE_LOGGING });
});
...
test('[Find Many] Get users with posts', () => {
	...
	const usersWithPosts = db._query.usersTable.findMany({
		with: {
			posts: true,
		},
	}).sync();
...
test('[Find Many] Get users with posts in transaction', () => {
	...
	db.transaction((tx) => {
		...
		usersWithPosts = tx._query.usersTable.findMany({
			where: (({ id }, { eq }) => eq(id, 1)),
			with: {
				posts: {
					where: (({ id }, { eq }) => eq(id, 1)),
...
test('[Find One] Get only custom fields', () => {
	...
	const usersWithPosts = db._query.usersTable.findFirst({
		columns: {},
		with: {
			posts: {
				columns: {},
...
test('Get user with invitee', () => {
	...
	const usersWithInvitee = db._query.usersTable.findMany({
		with: {
			invitee: true,
		},
	}).sync();
...
test('Get user with invitee and posts', () => {
	...
	const response = db._query.usersTable.findMany({
		with: {
			invitee: true,
			posts: true,
		},
...
test('[Find Many] Get groups with users', () => {
	...
	const response = db._query.groupsTable.findMany({
		with: {
			usersToGroups: {
				columns: {},
				with: {
...
test('[Find One] Get users with groups', () => {
	...
	const response = db._query.usersTable.findFirst({
		with: {
			usersToGroups: {
				columns: {},
				with: {
...
test('[Find One] Get groups with users', () => {
	...
	const response = db._query.groupsTable.findFirst({
		with: {
			usersToGroups: {
				columns: {},
				with: {
...
test('async api - prepare', async () => {
	...
	const queryStmt = db._query.usersTable.findMany().prepare();
