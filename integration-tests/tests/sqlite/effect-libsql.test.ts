...
import * as SQLiteDrizzle from 'drizzle-orm/effect-libsql';
...
const SQLiteClientLive = LibsqlClient.layer({
	// libsql's bare `:memory:` opens a fresh in-memory DB per connection, so a
	// committed transaction (which acquires its own connection) is invisible to
	// the base client afterwards. The shared cache makes all connections share
	// one in-memory DB.
