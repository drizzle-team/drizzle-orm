...
test.concurrent('migrator', async ({ db }) => {
	...
	await migrate(db as SQLiteCloudDatabase<typeof relations>, { migrationsFolder: './drizzle2/sqlite' });
...
test.concurrent('migrator : --init', async ({ db }) => {
	...
	const migratorRes = await migrate(db as SQLiteCloudDatabase<typeof relations>, {
...
test.concurrent('migrator : --init - db migrations error', async ({ db }) => {
	...
	await migrate(db as SQLiteCloudDatabase<typeof relations>, {
		migrationsFolder: './drizzle2/sqlite',
		migrationsTable,
	});
...
test.concurrent('migrator: local migration is unapplied. Migrations timestamp is less than last db migration', async ({ db }) => {
	...
	await migrate(db as SQLiteCloudDatabase<typeof relations>, { migrationsFolder: migrationDir });
