...
test('migrator', async ({ db }) => {
	...
	migrate(db as NodeSQLiteDatabase<typeof relations>, { migrationsFolder: './drizzle2/sqlite' });
...
test('migrator : --init', async ({ db }) => {
	...
	const migratorRes = migrate(db as NodeSQLiteDatabase<typeof relations>, {
...
test('migrator : --init - db migrations error', async ({ db }) => {
	...
	migrate(db as NodeSQLiteDatabase<typeof relations>, {
		migrationsFolder: './drizzle2/sqlite',
		migrationsTable,
	});
...
test('migrator: local migration is unapplied. Migrations timestamp is less than last db migration', async ({ db }) => {
	...
	await migrate(db as NodeSQLiteDatabase<typeof relations>, { migrationsFolder: migrationDir });
