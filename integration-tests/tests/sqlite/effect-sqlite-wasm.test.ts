...
import * as SQLiteDrizzle from 'drizzle-orm/effect-sqlite-wasm';
import { migrate } from 'drizzle-orm/effect-sqlite-wasm/migrator';
...
const SQLiteClientLive = SqliteClient.layerMemory({});
...
const bundleMigrations = (folder: string): Record<string, string> => {
	const migrations: Record<string, string> = {};
	for (const name of readdirSync(folder)) {
		const file = `${folder}/${name}/migration.sql`;
		if (existsSync(file)) migrations[name] = readFileSync(file, 'utf8');
	}
	return migrations;
};
	...
	addTests: (it) => {
		it.effect('migrator', () =>
			Effect.gen(function*() {
				...
				yield* migrate(db, { migrations: bundleMigrations('./drizzle2/sqlite') });
		...
		it.effect('migrator : --init', () =>
			Effect.gen(function*() {
				...
				const migratorRes = yield* migrate(db, {
					migrations: bundleMigrations('./drizzle2/sqlite'),
		...
		it.effect('migrator : --init - local migrations error', () =>
			Effect.gen(function*() {
				...
				const migratorRes = yield* migrate(db, {
					migrations: bundleMigrations('./drizzle2/sqlite-init'),
		...
		it.effect('migrator : local migration is unapplied. Migrations timestamp is less than last db migration', () =>
			Effect.gen(function*() {
				...
				yield* migrate(db, { migrations: bundleMigrations(migrationDir) });
