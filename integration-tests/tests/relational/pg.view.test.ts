...
import * as schemaTables from './pg.schema.ts';
import * as schema from './pg.view.schema.ts';
...
declare module 'vitest' {
	export interface TestContext {
		...
		pgDbView: NodePgDatabase<typeof schema>;
		pgClient: pg.Client;
	}
}
...
beforeEach(async (ctx) => {
	...
	await Promise.all([
		db.execute(sql`ALTER SEQUENCE "users_id_seq" RESTART WITH 1`),
		db.execute(sql`ALTER SEQUENCE "groups_id_seq" RESTART WITH 1`),
		db.execute(sql`ALTER SEQUENCE "users_to_groups_id_seq" RESTART WITH 1`),
		db.execute(sql`ALTER SEQUENCE "posts_id_seq" RESTART WITH 1`),
		db.execute(sql`ALTER SEQUENCE "comments_id_seq" RESTART WITH 1`),
		db.execute(sql`ALTER SEQUENCE "comment_likes_id_seq" RESTART WITH 1`),
	]);
});
...
test('[Find Many] Get users with posts + orderBy', async (t) => {
	...
	expect(usersWithPosts[2]).toEqual({
		...
		name: 'Dan',
		invitedBy: null,
		posts: [{ id: 3, ownerId: 1, content: '3' }, {
			id: 2,
			ownerId: 1,
			content: '2',
...
test('[Find Many] Get users with posts in transaction', async (t) => {
	...
	await db.transaction(async (tx) => {
		...
		usersWithPosts = await tx.query.usersView.findMany({
			where: (({ id }, { eq }) => eq(id, 1)),
			with: {
				posts: {
					where: (({ id }, { eq }) => eq(id, 1)),
...
test('[Find One] Get only custom fields', async (t) => {
	...
	const usersWithPosts = await db.query.usersView.findFirst({
		columns: {},
		with: {
			posts: {
				columns: {},
...
test('[Find Many] Get select {}', async (t) => {
	...
	await expect(async () =>
		await db.query.usersView.findMany({
			columns: {},
		})
...
test('[Find One] Get select {}', async (t) => {
	...
	await expect(async () =>
		await db.query.usersView.findFirst({
			columns: {},
		})
	).rejects.toThrow(DrizzleError);
});
...
test('[Find Many] Get users with posts + prepared + limit + offset + where', async (t) => {
	...
	const prepared = db.query.usersView.findMany({
		limit: sql.placeholder('uLimit'),
		offset: sql.placeholder('uOffset'),
		where: (({ id }, { eq, or }) => or(eq(id, sql.placeholder('id')), eq(id, 3))),
		with: {
			posts: {
				where: (({ id }, { eq }) => eq(id, sql.placeholder('pid'))),
				limit: sql.placeholder('pLimit'),
			},
		},
	}).prepare('query4');
...
test('Get user with invitee', async (t) => {
	...
	const usersWithInvitee = await db.query.usersView.findMany({
		with: {
			invitee: true,
		},
	});
	...
	const response = await db.query.usersView.findMany({
		with: {
			usersToGroups: {
				...
				orderBy: usersToGroupsView.userId,
			},
		},
		orderBy: usersView.id,
	});
...
test('[Find Many] Get groups with users', async (t) => {
	...
	const response = await db.query.groupsView.findMany({
		with: {
			usersToGroups: {
				columns: {},
				with: {
...
test('[Find One] Get users with groups', async (t) => {
	...
	const response = await db.query.usersView.findFirst({
		with: {
			usersToGroups: {
				columns: {},
				with: {
...
test('[Find One] Get groups with users', async (t) => {
	...
	const response = await db.query.groupsView.findFirst({
		with: {
			usersToGroups: {
				columns: {},
				with: {
	...
	const response = await db.query.usersView.findMany({
		...
		with: {
			usersToGroups: {
				...
				orderBy: usersToGroupsView.groupId,
			},
		},
	});
