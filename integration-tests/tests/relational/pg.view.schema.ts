...
export const groupsView = pgView('groups_view').as((qb) =>
	qb.select({
		id: groupsTable.id,
...
export const usersToGroupsView = pgView('users_to_groups_view').as((qb) =>
	qb.select({
...
export const postsView = pgView('posts_view').as((qb) =>
	qb.select({
		id: postsTable.id,
...
export const commentsView = pgView('comments_view').as((qb) =>
	qb.select({
		id: commentsTable.id,
...
export const commentLikesView = pgView('comment_likes_view').as((qb) =>
	qb.select({
