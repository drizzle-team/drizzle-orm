...
import * as schemaTables from './mysql.schema.ts';
import * as schema from './mysql.view.schema.ts';
...
declare module 'vitest' {
	export interface TestContext {
		...
		mysqlDbView: MySql2Database<typeof schema>;
...
beforeAll(async () => {
	...
	Promise.all([
		...
		db.execute(sql`drop view if exists \`users_view\``),
		db.execute(sql`drop view if exists \`groups_view\``),
		db.execute(sql`drop view if exists \`users_to_groups_view\``),
		db.execute(sql`drop view if exists \`posts_view\``),
		db.execute(sql`drop view if exists \`comments_view\``),
		db.execute(sql`drop view if exists \`comment_likes_view\``),
	]);
...
test('[Find Many] Get users with groups + limit + where', async (t) => {
	...
	const response = await db.query.usersView.findMany({
		limit: 1,
		where: (_, { eq, or }) => or(eq(usersView.id, 1), eq(usersTable.id, 2)),
		with: {
			usersToGroups: {
