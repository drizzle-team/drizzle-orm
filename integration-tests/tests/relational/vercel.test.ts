...
beforeEach(async (ctx) => {
	...
	await ctx.vpgDbV2.execute(sql`drop schema public cascade`);
	await ctx.vpgDbV2.execute(sql`create schema public`);
	await ctx.vpgDbV2.execute(
		sql`
			CREATE TABLE "users" (
...
test('[Find Many] Get users with groups + orderBy', async (t) => {
	...
	await db.insert(usersToGroupsTable).values([
		{ userId: 1, groupId: 1, id: 1 },
		{ userId: 2, groupId: 2, id: 2 },
		{ userId: 3, groupId: 2, id: 3 },
		{ userId: 3, groupId: 3, id: 4 },
	]);
