...
import type { CockroachDbColumn, CockroachDbDatabase, CockroachDbQueryResultHKT } from 'drizzle-orm/cockroachdb-core';
import {
	alias,
	bigint,
	boolean,
...
declare module 'vitest' {
	interface TestContext {
		cockroachdb: {
			db: CockroachDbDatabase<CockroachDbQueryResultHKT>;
		};
	}
}

const en = cockroachdbEnum('en', ['enVal1', 'enVal2']);

const allTypesTable = cockroachdbTable('all_types', {
	int4: int4('int4'),
...
export const usersTable = cockroachdbTable('users', {
...
const usersOnUpdate = cockroachdbTable('users_on_update', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
...
const citiesTable = cockroachdbTable('cities', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
	state: char('state', { length: 2 }),
});

const cities2Table = cockroachdbTable('cities', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
});

const users2Table = cockroachdbTable('users2', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
	cityId: int4('city_id').references(() => citiesTable.id),
});

const coursesTable = cockroachdbTable('courses', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
	categoryId: int4('category_id').references(() => courseCategoriesTable.id),
});

const courseCategoriesTable = cockroachdbTable('course_categories', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
});

const orders = cockroachdbTable('orders', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
...
const network = cockroachdbTable('network_table', {
	inet: inet('inet').notNull(),
});

const salEmp = cockroachdbTable('sal_emp', {
	name: text('name'),
	payByQuarter: int4('pay_by_quarter').array(),
});

export const usersMigratorTable = cockroachdbTable('users12', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
	name: text('name').notNull(),
...
const aggregateTable = cockroachdbTable('aggregate_table', {
...
export const mySchema = cockroachdbSchema('mySchema');
...
const jsonTestTable = cockroachdbTable('jsontest', {
	id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
...
export function tests() {
	describe('common', () => {
		...
		async function setupSetOperationTest(db: CockroachDbDatabase<CockroachDbQueryResultHKT>) {
		...
		async function setupAggregateFunctionsTest(db: CockroachDbDatabase<CockroachDbQueryResultHKT>) {
		...
		test('table configs: unique third param', async () => {
			const cities1Table = cockroachdbTable(
				'cities1',
				{
		...
		test('table configs: unique in column', async () => {
			const cities1Table = cockroachdbTable('cities1', {
				id: int4('id').primaryKey(),
		...
		test('table config: foreign keys name', async () => {
			const table = cockroachdbTable('cities', {
				id: int4('id'),
				name: text('name').notNull(),
				state: text('state'),
		...
		test('select distinct', async (ctx) => {
			...
			const usersDistinctTable = cockroachdbTable('users_distinct', {
				id: int4('id').notNull(),
				name: text('name').notNull(),
		...
		test('full join with alias', async (ctx) => {
			...
			const cockroachdbTable = cockroachdbTableCreator((name) => `prefixed_${name}`);
		...
		test('Insert all defaults in 1 row', async (ctx) => {
			...
			const users = cockroachdbTable('empty_insert_single', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				name: text('name').default('Dan'),
				state: text('state'),
			});
		...
		test('Insert all defaults in multiple rows', async (ctx) => {
			...
			const users = cockroachdbTable('empty_insert_multiple', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				name: text('name').default('Dan'),
				state: text('state'),
			});
		...
		test('with ... update', async (ctx) => {
			...
			const products = cockroachdbTable('products', {
		...
		test('select count w/ custom mapper', async (ctx) => {
			...
			function count(value: CockroachDbColumn | SQLWrapper): SQL<number>;
			function count(value: CockroachDbColumn | SQLWrapper, alias: string): SQL.Aliased<number>;
			function count(value: CockroachDbColumn | SQLWrapper, alias?: string): SQL<number> | SQL.Aliased<number> {
		...
		test('view', async (ctx) => {
			...
			const newYorkers1 = cockroachdbView('new_yorkers')
				.as((qb) => qb.select().from(users2Table).where(eq(users2Table.cityId, 1)));

			const newYorkers2 = cockroachdbView('new_yorkers', {
				id: int4('id').primaryKey(),
				name: text('name').notNull(),
				cityId: int4('city_id').notNull(),
			}).as(sql`select * from ${users2Table} where ${eq(users2Table.cityId, 1)}`);

			const newYorkers3 = cockroachdbView('new_yorkers', {
				id: int4('id').primaryKey(),
				name: text('name').notNull(),
				cityId: int4('city_id').notNull(),
			}).existing();
		...
		test('materialized view', async (ctx) => {
			...
			const newYorkers1 = cockroachdbMaterializedView('new_yorkers')
				.as((qb) => qb.select().from(users2Table).where(eq(users2Table.cityId, 1)));
			...
			const newYorkers3 = cockroachdbMaterializedView('new_yorkers', {
				id: int4('id').primaryKey(),
				name: text('name').notNull(),
				cityId: int4('city_id').notNull(),
			}).existing();
		...
		test('select from existing view', async (ctx) => {
			...
			const schema = cockroachdbSchema('test_schema');
		...
		test('prefixed table', async (ctx) => {
			...
			const cockroachdbTable = cockroachdbTableCreator((name) => `myprefix_${name}`);

			const users = cockroachdbTable('test_prefixed_table_with_unique_name', {
				id: int4('id').primaryKey(),
				name: text('name').notNull(),
			});
		...
		test('select from enum as ts enum', async (ctx) => {
			...
			const muscleEnum = cockroachdbEnum('muscle', Muscle);

			const forceEnum = cockroachdbEnum('force', Force);

			const levelEnum = cockroachdbEnum('level', Level);

			const mechanicEnum = cockroachdbEnum('mechanic', Mechanic);

			const equipmentEnum = cockroachdbEnum('equipment', Equipment);

			const categoryEnum = cockroachdbEnum('category', Category);

			const exercises = cockroachdbTable('exercises', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
		...
		test('select from enum', async (ctx) => {
			...
			const muscleEnum = cockroachdbEnum('muscle', [
				'abdominals',
				'hamstrings',
				'adductors',
				'quadriceps',
			...
			const forceEnum = cockroachdbEnum('force', ['isometric', 'isotonic', 'isokinetic']);

			const levelEnum = cockroachdbEnum('level', ['beginner', 'intermediate', 'advanced']);

			const mechanicEnum = cockroachdbEnum('mechanic', ['compound', 'isolation']);

			const equipmentEnum = cockroachdbEnum('equipment', [
				'barbell',
				'dumbbell',
				'bodyweight',
				'machine',
			...
			const categoryEnum = cockroachdbEnum('category', ['upper_body', 'lower_body', 'full_body']);
		...
		test('all date and time columns', async (ctx) => {
			...
			const table = cockroachdbTable('all_columns', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
		...
		test('select from sql', async (ctx) => {
			...
			const metricEntry = cockroachdbTable('metric_entry', {
		...
		test('timestamp timezone', async (ctx) => {
			...
			const usersTableWithAndWithoutTimezone = cockroachdbTable('users_test_with_and_without_timezone', {
				id: int4('id').primaryKey().generatedAlwaysAsIdentity(),
				name: text('name').notNull(),
				createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
		...
		test('transaction', async (ctx) => {
			...
			const users = cockroachdbTable('users_transactions', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				balance: int4('balance').notNull(),
			});
			const products = cockroachdbTable('products_transactions', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
		...
		test('transaction rollback', async (ctx) => {
			...
			const users = cockroachdbTable('users_transactions_rollback', {
				id: int4('id').primaryKey().generatedAlwaysAsIdentity(),
				balance: int4('balance').notNull(),
			});
		...
		test('nested transaction', async (ctx) => {
			...
			const users = cockroachdbTable('users_nested_transactions', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				balance: int4('balance').notNull(),
			});
		...
		test('nested transaction rollback', async (ctx) => {
			...
			const users = cockroachdbTable('users_nested_transactions_rollback', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				balance: int4('balance').notNull(),
			});
		...
		test('subquery with view', async (ctx) => {
			...
			const users = cockroachdbTable('users_subquery_view', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				name: text('name').notNull(),
				cityId: int4('city_id').notNull(),
			});

			const newYorkers = cockroachdbView('new_yorkers').as((qb) => qb.select().from(users).where(eq(users.cityId, 1)));
		...
		test('join view as subquery', async (ctx) => {
			...
			const users = cockroachdbTable('users_join_view', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				name: text('name').notNull(),
				cityId: int4('city_id').notNull(),
			});
		...
		test('array mapping and parsing', async (ctx) => {
			...
			const arrays = cockroachdbTable('arrays_tests', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				tags: text('tags').array(),
		...
		test('proper json and jsonb handling', async (ctx) => {
			...
			const jsonTable = cockroachdbTable('json_table', {
		...
		test('update ... from with join', async (ctx) => {
			...
			const states = cockroachdbTable('states', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
				name: text('name').notNull(),
			});
		...
		test('insert into ... select', async (ctx) => {
			...
			const notifications = cockroachdbTable('notifications', {
				id: int4('id').primaryKey().generatedByDefaultAsIdentity(),
			...
			const userNotications = cockroachdbTable('user_notifications', {
		...
		test('insert into ... select with keys in different order', async (ctx) => {
			...
			const users1 = cockroachdbTable('users1', {
				id: int4('id').primaryKey(),
				name: text('name').notNull(),
			});
		...
		test('policy', () => {
			{
				...
				expect(is(policy, CockroachDbPolicy)).toBe(true);
			...
			{
				const p1 = cockroachdbPolicy('test policy');
				const p2 = cockroachdbPolicy('test policy 2', {
					as: 'permissive',
					for: 'all',
					to: 'public',
					using: sql`1=1`,
				...
				const table = cockroachdbTable('table_with_policy', {
					id: int4('id').primaryKey(),
					name: text('name').notNull(),
				}, () => [
					p1,
		...
		test('Enable RLS function', () => {
			const usersWithRLS = cockroachdbTable('users', {
				id: int4(),
			}).enableRLS();

			const config1 = getTableConfig(usersWithRLS);

			const usersNoRLS = cockroachdbTable('users', {
				id: int4(),
			});
		...
		test('$count separate', async (ctx) => {
			...
			const countTestTable = cockroachdbTable('count_test', {
				id: int4('id').notNull(),
				name: text('name').notNull(),
			});
		...
		test('insert multiple rows into table with generated identity column', async (ctx) => {
			...
			const identityColumnsTable = cockroachdbTable('identity_columns_table', {
		...
		test('generated always columns', async (ctx) => {
			...
			const genColumns = cockroachdbTable('gen_columns', {
				id: int4(),
