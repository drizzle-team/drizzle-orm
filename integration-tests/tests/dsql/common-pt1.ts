...
async function resetDeepNullification(db: DsqlAsyncDatabase<any, any>) {
...
async function runDeepNullification(db: DsqlAsyncDatabase<any, any>) {
	...
	await db.insert(dnTicket).values([{ id: 1, staffId: 1 }, { id: 2, staffId: 2 }, { id: 3, staffId: 3 }]); // #3 -> outer miss
	await db.insert(dnVStaff).values([{ id: 1, deptId: 1 }, { id: 2, deptId: 1 }]);
	await db.insert(dnDept).values([{ id: 1, name: 'Eng' }]);
	await db.insert(dnEmp).values([{ id: 1, staffId: 1 }, { id: 2, staffId: 2 }, { id: 3, staffId: 3 }]); // #3 -> outer view miss
...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.concurrent('select with empty array in inArray', async ({ db, push }) => {
			...
			await db.insert(users).values([{ id: 1, name: 'John' }, { id: 2, name: 'Jane' }, { id: 3, name: 'Jane' }]);
		...
		test.concurrent('select distinct', async ({ db, push }) => {
			...
			const users4 = await db.selectDistinctOn([usersDistinctTable.id, usersDistinctTable.age]).from(
				usersDistinctTable,
			).orderBy(usersDistinctTable.id, usersDistinctTable.age, desc(usersDistinctTable.name));
		...
		test.concurrent('update with returning all fields', async ({ db, push }) => {
			...
			expect(Math.abs(usersResult[0]!.createdAt.getTime() - now)).toBeLessThan(5000);
		...
		test.concurrent('json insert', async ({ db, push }) => {
			...
			await db.insert(users).values({ id: 1, name: 'John', jsonb: ['foo', 'bar'] });
			const result = await db
				.select({
					id: users.id,
					name: users.name,
		...
		test.concurrent('char insert', async ({ db, push }) => {
			...
			await db.insert(cities).values({ id: 1, name: 'Austin', state: 'TX' });
			const result = await db
		...
		test.concurrent('insert with explicit column list', async ({ db, push }) => {
			...
			await db.insert(table, 'id', 'note', 'name').values({ id: 4, name: 'Jill', note: 'hi' });
		...
		test.concurrent('insert with explicit column list - select', async ({ db, push }) => {
			...
			await db.insert(dst, 'id', 'name').select(
				db.select({ id: src.id, name: src.name }).from(src).orderBy(src.id),
			);
		...
		test.concurrent('insert sql', async ({ db, push }) => {
			...
			await db.insert(users).values({ id: 1, name: sql`${'John'}` });
		...
		test.concurrent('Nullify all-null group from from nullable join', async ({ db, push }) => {
			...
			await db.insert(citiesTable).values([{ id: 1, name: 'Paris', state: 'IDF', zip: '75' }, {
				id: 2,
				name: 'London',
			}]);
		...
		test.concurrent("Don't disregard added SQL field during join nullification", async ({ db, push }) => {
			...
			await db.insert(citiesTable).values([{ id: 1, name: 'Paris', state: 'IDF' }, { id: 2, name: 'London' }]);
			await db.insert(users2Table).values([{ id: 1, name: 'John', cityId: 1 }, { id: 2, name: 'Jane', cityId: 2 }]);
		...
		test.concurrent('issue #4917', async ({ db, push }) => {
			const featureFlag = pgTable('feature_flag', (d) => ({
				// DSQL has no sequences, so no identity columns either
			...
			const featureFlagSetting = pgTable('feature_flag_setting', (d) => ({
				...
				featureFlagId: d
					.integer()
					.notNull(),
				// DSQL has no user-defined types, so the enum is a plain text column here
				scope: d.text().$type<'global' | 'user'>().notNull(), // These become problematic since the are defined without name
			...
			const schema = { featureFlag, featureFlagSetting, featureFlagView };
		...
		test.concurrent('RQB v2 numeric precision preserved in nested relation', async ({ push, createDB }) => {
			...
			const products = pgTable('rqb_numeric_products', {
				...
				// a bare `numeric` is `numeric(18,6)` on DSQL - the scales match the values below exactly
				priceUahRetail: numeric('price_uah_retail', { precision: 10, scale: 4 }),
				priceUahWholesaleBig: numeric('price_uah_wholesale_big', { precision: 50, scale: 24 }),
			});
		...
		test.concurrent('Issue No1117', async ({ push, db }) => {
			...
			const result = await db.select().from(table).orderBy(sql`json_typeof(${table.jsonData})`);
