...
export function tests(test: Test) {
	describe('caches', () => {
		test.beforeEach(async ({ caches }) => {
			...
			for (const table of ['users', 'posts', 'short-link', 'somethingelse_error_wrap_slowpath']) {
				await explicit.execute(sql`drop table if exists ${sql.identifier(table)} cascade`);
			}
		...
		test('select+join+with', async ({ caches, push }) => {
			...
			const shortLink = pgTable('short-link', {
				...
				// DSQL has no foreign keys - the column is kept, the constraint isn't
				createdBy: integer(),
			});
