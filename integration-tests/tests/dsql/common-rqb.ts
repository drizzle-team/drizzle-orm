...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.concurrent('RQB v2 find many - table creation func', async ({ push, createDB }) => {
			...
			await db.insert(orders).values([{ id: 1, userId: 1, amount: 11, status: 'delivered' }, {
				id: 2,
				userId: 2,
				amount: 22,
				status: 'delivered',
		...
		test.concurrent(
			...
			async ({ push, createDB }) => {
				...
				const orders = pgTable('rqb_orders_18', {
					id: integer('id').primaryKey(),
					userId: integer('user_id').notNull(),
					status: text('status').$type<'IN_PROGRESS' | 'CANCELED' | 'CLOSED'>(),
				});
				...
				await db.insert(orders).values([{ id: 1, userId: 1, status: 'CANCELED' }, {
					id: 2,
					userId: 2,
					status: 'IN_PROGRESS',
				}]);
	...
	test.concurrent('issue #4169', async ({ createDB }) => {
		...
		const jobCandidacies = pgTable('job_candidacy', {
			...
			candidateId: uuid('candidate_id')
				.notNull(),
			status: text().$type<'IN_PROGRESS' | 'CANCELED' | 'CLOSED'>().notNull(),
		});
	...
	test.concurrent('issue No3943', async ({ createDB, push }) => {
		const parent = pgTable('parent', {
			id: integer().primaryKey(),
			// a bare `decimal` is `numeric(18,6)` on DSQL, which pads the scale
			parentVal: decimal({ precision: 10, scale: 2 }),
		});
		const child = pgTable('child', {
			parentId: integer(),
			childVal: decimal({ precision: 10, scale: 2 }),
		});
		...
		const [parentRes] = await db.insert(parent).values({ id: 1, parentVal: '12.34' }).returning();
	...
	test.concurrent('Issue No2279', async ({ createDB, push }) => {
		...
		await db.execute(sql`INSERT INTO corrupt_jsonb_demo (id, data) VALUES (1, '{"a": 1}');`);
