...
import { defineRelations, DrizzleError, eq, type SQL, sql, TransactionRollbackError } from 'drizzle-orm';
import { drizzle, type NodePgDsqlDatabase } from 'drizzle-orm/node-postgres/dsql';
...
declare module 'vitest' {
	export interface TestContext {
		dsqlDbV2: NodePgDsqlDatabase<typeof relations>;
		dsqlClient: AuroraDSQLPool;
	}
}

let db: NodePgDsqlDatabase<typeof relations>;
let client: AuroraDSQLPool;
...
beforeAll(async () => {
	...
	do {
		try {
			client = new AuroraDSQLPool({ connectionString, max: 1 });
...
beforeEach(async (ctx) => {
	ctx.dsqlDbV2 = db;
	ctx.dsqlClient = client;

	// setup DDL occasionally hits OCC conflicts on DSQL - see `retryOcc`
	const run = (query: SQL) => retryOcc(() => ctx.dsqlDbV2.execute(query));
	...
	const usersWithPosts = await db.query.usersTable.findFirst({
		...
		with: {
			posts: {
				...
				extras: ({
					contentLower: ({ content }) => sql<string>`lower(${content})`.as('content_lower'),
				}),
			},
		},
		where: {
...
test('[Find Many .through] Get groups with users with posts where', async (t) => {
	...
	await db.insert(postsTable).values([
		{ id: 1, ownerId: 1, content: 'Post1' },
		{ id: 2, ownerId: 1, content: 'Post2' },
		{ id: 3, ownerId: 1, content: 'Post3' },
		{ id: 4, ownerId: 2, content: 'Post1' },
		{ id: 5, ownerId: 2, content: 'Post2' },
		{ id: 6, ownerId: 3, content: 'Post1' },
		{ id: 7, ownerId: 3, content: 'Post2' },
	]);
