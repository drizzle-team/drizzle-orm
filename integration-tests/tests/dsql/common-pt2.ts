...
import type { DsqlAsyncSession } from 'drizzle-orm/pg-core/async/dsql';
import { PostgresJsDsqlDatabase } from 'drizzle-orm/postgres-js/dsql';
...
export function tests(test: Test) {
	describe('common', () => {
		...
		test.concurrent(
			...
			async ({ db, push }) => {
				...
				await db.insert(recipes).values({ id: 1 });
				await db.insert(creators).values({ id: 1 });
		...
		test.concurrent('aggregate function: count', async ({ db, push }) => {
			...
			await db.insert(aggregateTable).values([
				{ id: 1, name: 'value 1', value: 10 },
				{ id: 2, name: 'value 1', value: 20 },
				{ id: 3, name: 'value 2', value: 50 },
				{ id: 4, name: 'value 3', value: 20 },
				{ id: 5, name: 'value 4', value: 90 },
				{ id: 6, name: 'value 5', value: 10 },
				{ id: 7, name: 'value 6', nullOnly: null },
			]);
		...
		test.concurrent('mySchema :: insert + select', async ({ db, push }) => {
			...
			await db.insert(users).values({ id: 2, name: 'Jane' });
		...
		test.concurrent(
			...
			async ({ db, push }) => {
				...
				await db.insert(users).values({ id: 1, name: 'John', verified: true });
				const result = await db.select().from(users);
		...
		test.concurrent(
			...
			async ({ db, push }) => {
				...
				await db
					.insert(users)
					.values([{ id: 1, name: 'John' }, { id: 2, name: 'Jane' }, { id: 3, name: 'Jane' }]);
		...
		test.concurrent('mySchema :: insert with spaces', async ({ db, push }) => {
			...
			await db.insert(users).values({ id: 1, name: sql`'Jo   h     n'` });
			const result = await db
				.select({ id: users.id, name: users.name })
				.from(users);
		...
		test.concurrent(
			...
			async ({ db }) => {
				...
				expect(query).toEqual({
					sql:
						'insert into "mySchema_15"."users" ("id", "name", "verified", "jsonb", "created_at") values ($1, $2, default, $3, default) on conflict ("id","name") do update set "name" = $4',
					params: [
						1,
						'John',
						is(db, PostgresJsDsqlDatabase)
							? ['foo', 'bar']
		...
		test.concurrent(
			...
			async ({ db }) => {
				...
				expect(query).toEqual({
					sql:
						'insert into "mySchema_16"."users" ("id", "name", "verified", "jsonb", "created_at") values ($1, $2, default, $3, default) on conflict ("id") do nothing',
					params: [
						1,
						'John',
						is(db, PostgresJsDsqlDatabase)
		...
		test.concurrent('mySchema :: view', async ({ db, push }) => {
			...
			await db.insert(cities).values([{ id: 1, name: 'New York' }, { id: 2, name: 'Paris' }]);
		...
		test.concurrent('Object keys as column names', async ({ db, push }) => {
			...
			await db.insert(users).values([
				{ id: 1, firstName: 'John', lastName: 'Doe', admin: true },
				{ id: 2, firstName: 'Jane', lastName: 'Smith', admin: false },
			]);
			const result = await db
				.select({
					id: users.id,
		...
		test.concurrent('update ... from', async ({ db, push }) => {
			...
			await db
				.insert(cities2Table)
				.values([{ id: 1, name: 'New York City' }, { id: 2, name: 'Seattle' }]);
			await db.insert(users2Table).values([
				{ id: 1, name: 'John', cityId: 1 },
				{ id: 2, name: 'Jane', cityId: 2 },
			]);
		...
		test.concurrent('update ... from with join', async ({ db, push }) => {
			...
			await db
				.insert(states)
				.values([{ id: 1, name: 'New York' }, { id: 2, name: 'Washington' }]);
			await db
				.insert(cities)
				.values([
		...
		test.concurrent('insert into ... select', async ({ db, push }) => {
			...
			const newNotification = await db
				...
				.values({ id: 1, message: 'You are one of the 3 lucky winners!' })
		...
		test.concurrent('insert as cte', async ({ db, push }) => {
			...
			const sq1 = (id: number) =>
				db
					.$with('sq')
					.as(db.insert(users).values({ id, name: 'John' }).returning());
			const sq1a = sq1(1);
			const sq1b = sq1(2);
			const result1 = await db.with(sq1a).select().from(sq1a);
			const result2 = await db.with(sq1b).select({ id: sq1b.id }).from(sq1b);
			...
			const sq2a = sq2(3);
			const sq2b = sq2(4);
			const result3 = await db.with(sq2a).select().from(sq2a);
			const result4 = await db.with(sq2b).select({ name: sq2b.name }).from(sq2b);
		...
		test.concurrent('delete as cte', async ({ db, push }) => {
			...
			const result1 = await db.with(sq1).select().from(sq1);
			await db.insert(users).values({ id: 3, name: 'John' });
			...
			const result3 = await db.with(sq2).select().from(sq2);
			await db.insert(users).values({ id: 4, name: 'Jane' });
		...
		test.concurrent('cross join', async ({ db, push }) => {
			...
			await db.insert(usersTable).values([{ id: 1, name: 'John' }, { id: 2, name: 'Jane' }]);

			await db
				.insert(citiesTable)
				.values([{ id: 1, name: 'Seattle' }, { id: 2, name: 'New York City' }]);
		...
		test.concurrent('left join (lateral)', async ({ db, push }) => {
			...
			await db
				.insert(users2Table)
				.values([{ id: 1, name: 'John', cityId: 1 }, { id: 2, name: 'Jane' }]);
		...
		test.concurrent('.where with isNull in it', async ({ db, push }) => {
			...
			const res = await query.orderBy(desc(table.col1));
			expect(res).toStrictEqual([
				{ col1: true, col2: null },
		...
		test.concurrent(
			...
			async ({ db, push }) => {
				...
				expect(updateResp[0]!.updatedAt.getTime()).greaterThan(insertResp[0]!.updatedAt.getTime());
			},
		);
		...
		test.concurrent('all types', async ({ db, push }) => {
			const { allTypesTable } = makeAllTypes('all_types_48');
		...
		test.concurrent('all types ~codecs~', async ({ createDB, push }) => {
			...
			const session = (<any> db).session as DsqlAsyncSession;
		...
		test('casing in sql``', async ({ db, push }) => {
			const payments = snakeCase.table('payments', {
				id: integer().primaryKey(),
				// a bare `numeric` is `numeric(18,6)` on DSQL, which pads the scale
		...
		test.concurrent('Mappers: select complex selections', async ({ db, push }) => {
			...
			const notes = pgTable('mappers_notes_1', (t) => ({
				id: t.integer('id').primaryKey(),
				ownerId: t.bigint('owner_id', { mode: 'number' }),
				body: t.text('body'),
			}));
		...
		test.concurrent('Jit mappers: select complex selections', async ({ createDB, push }) => {
			...
			const notes = pgTable('jit_mappers_notes_1', (t) => ({
				id: t.integer('id').primaryKey(),
				ownerId: t.bigint('owner_id', { mode: 'number' }),
				body: t.text('body'),
			}));
		...
		test.concurrent('$onUpdateFn called only when needed', async ({ db, push }) => {
			let counter = 0;
			const table = pgTable('on_update_call_test', (t) => ({
				id: t.integer().primaryKey(),
				name: t.text().notNull(),
				inc: t.integer().$onUpdateFn(() => counter++),
			}));
	...
	describe('transaction config', () => {
		const occError = (code: 'OC000' | 'OC001' | '40001' = 'OC000') =>
			Object.assign(new Error('change conflicts with another transaction'), { code });

		test('transaction with occ retries (conflict resolved)', async ({ db, push }) => {
			const users = pgTable('users_tx_occ_retry', {
				id: integer('id').primaryKey(),
			...
			const result = await db.transaction(async (tx) => {
				++attempts;
				// re-inserting the same key on every attempt proves each failed attempt was rolled back
				await tx.insert(users).values({ id: 1, balance: 100 });
				if (attempts === 1) throw occError('OC000');
				if (attempts === 2) throw new DrizzleQueryError('update "users_tx_occ_retry"', [], occError('40001'));
				return 'done';
			}, { occ: true });

			expect(result).toBe('done');
			expect(attempts).toBe(3);
			expect(await db.select().from(users)).toEqual([{ id: 1, balance: 100 }]);
		});

		test('transaction with occ retries (retries exhausted)', async ({ db }) => {
			let attempts = 0;
			await expect(db.transaction(async () => {
				++attempts;
				throw occError();
			}, { occ: { maxRetries: 2, baseDelayMs: 1, maxDelayMs: 2 } })).rejects.toThrowError(/change conflicts/);

			expect(attempts).toBe(3);
		});

		test('transaction with occ retries disabled', async ({ db }) => {
			let attempts = 0;
			await expect(db.transaction(async () => {
				++attempts;
				throw occError();
			}, { occ: false })).rejects.toThrowError(/change conflicts/);

			expect(attempts).toBe(1);

			// retries are opt-in, even where the connector would retry on its own
			let omittedAttempts = 0;
			await expect(db.transaction(async () => {
				++omittedAttempts;
				throw occError();
			})).rejects.toThrowError(/change conflicts/);

			expect(omittedAttempts).toBe(1);

			let emptyConfigAttempts = 0;
			await expect(db.transaction(async () => {
				++emptyConfigAttempts;
				throw occError();
			}, {})).rejects.toThrowError(/change conflicts/);

			expect(emptyConfigAttempts).toBe(1);
		});

		test('transaction with occ retries (non-conflict error)', async ({ db }) => {
			let attempts = 0;
			await expect(db.transaction(async () => {
				++attempts;
				throw new Error('not a conflict');
			}, { occ: true })).rejects.toThrowError(/not a conflict/);

			expect(attempts).toBe(1);
		});

		test('transaction with occ retries (invalid config)', async ({ db }) => {
			let attempts = 0;
			await expect(db.transaction(async () => {
				++attempts;
			}, { occ: { maxRetries: -1 } })).rejects.toThrowError(/maxRetries must be >= 0/);
		...
		test('nested transaction', async ({ db }) => {
			...
			).rejects.toThrowError(/Aurora DSQL does not support nested transactions/);
		});
		...
		test('transaction with an empty options object', async ({ db, push }) => {
			...
			await push({ users });
			await db.insert(users).values({ id: 1, balance: 100 });
