...
import type { BunSQLSQLiteDatabase } from 'drizzle-orm/bun-sql/sqlite';
...
let db: BunSQLSQLiteDatabase;
let client: BunSQL;

beforeEach(() => {
	client = new BunSQL(':memory:');
...
test('insert bigint values', async () => {
	...
	await db.insert(bigIntExample).values({ name: 'two', bigInt: BigInt('127') });
	await db.insert(bigIntExample).values({ name: 'three', bigInt: BigInt('32767') });
	await db.insert(bigIntExample).values({ name: 'four', bigInt: BigInt('1234567890') });
	await db.insert(bigIntExample).values({ name: 'five', bigInt: BigInt('12345678900987654321') });

	const result = await db.select().from(bigIntExample);
...
test.skip('insert via db.get', async () => {
	console.trace('insert via db.get');
...
test.skip('insert via db.run + select via db.get', async () => {
	await db.run(sql`insert into ${usersTable} (${new Name(usersTable.name.name)}) values (${'John'})`);
...
test.skip('insert via db.get w/ query builder', async () => {
...
test('select from a many subquery', async () => {
...
test('select from a one subquery', async () => {
	await db.insert(citiesTable)
		.values([{ name: 'Paris' }, { name: 'London' }]);
...
test.skip('insert undefined', async () => {
	...
	await expect((async () => {
		try {
			console.trace('inserting undefined');
			await db.insert(users).values({ name: undefined }).run();
			console.trace('inserted undefined');
		} catch (error) {
			console.trace(error);
			throw error;
		}
...
test('aggregate function: count', async () => {
	...
	await setupAggregateFunctionsTest();
...
test('test $onUpdateFn and $onUpdate works with sql value', async () => {
	const users = sqliteTable('users', {
		id: integer('id').primaryKey({ autoIncrement: true }),
		name: text('name').notNull(),
	...
	expect(insertResp[0]?.updatedAt ?? 0).toBeLessThan(now);
	expect(updateResp[0]?.updatedAt ?? 0).toBeGreaterThan(now);
});
