...
import { alias, bigint, cockroachTable, int4 } from 'drizzle-orm/cockroach-core';
import type { Pool } from 'pg';
import { afterAll, beforeAll, beforeEach, expect, expectTypeOf, test } from 'vitest';
import relations from './cockroach.relations';
import {
	allTypesTable,
	commentsTable,
	courseOfferings,
	...
	usersTable,
	usersToGroupsTable,
} from './cockroach.schema';

const ENABLE_LOGGING = false;

declare module 'vitest' {
	export interface TestContext {
		cockroachDbV2: NodeCockroachDatabase<typeof relations>;
		cockroachClient: Pool;
	}
}

let db: NodeCockroachDatabase<typeof relations> & { $client: Pool };
...
beforeAll(async () => {
	...
	if (!connected) {
		console.error('Cannot connect to CockroachDB');
...
beforeEach(async (ctx) => {
	ctx.cockroachDbV2 = db;
	ctx.cockroachClient = db.$client;

	await ctx.cockroachDbV2.execute(sql`drop database defaultdb`);
	await ctx.cockroachDbV2.execute(sql`create database defaultdb`);
	await ctx.cockroachDbV2.execute(sql`create schema rqb_test_schema`);
	await ctx.cockroachDbV2.execute(
		sql`
			CREATE TABLE "users" (
	...
	await ctx.cockroachDbV2.execute(
		sql`
			CREATE TABLE "students" (
				"student_id" int4 PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
				"name" text NOT NULL
			);
		`,
	);
...
test('[Find Many] Get only custom fields', async (t) => {
	...
	expect(usersWithPosts).toContainEqual({
		lowerName: 'dan',
		posts: expect.arrayContaining([{ lowerName: 'post1' }, {
			lowerName: 'post1.2',
		}, { lowerName: 'post1.3' }]),
	});
	expect(usersWithPosts).toContainEqual({
		lowerName: 'andrew',
		posts: expect.arrayContaining([{ lowerName: 'post2' }, {
			lowerName: 'post2.1',
		}]),
	});
	expect(usersWithPosts).toContainEqual({
		lowerName: 'alex',
		posts: expect.arrayContaining([{ lowerName: 'post3' }, {
			lowerName: 'post3.1',
		}]),
	});
});
...
test('alltypes', async () => {
	...
	type ExpectedType = {
		serial: number | null;
		bigserial53: number | null;
		bigserial64: bigint | null;
		int: number | null;
...
(() => {
	...
	const brandtypes = cockroachTable('users', (t) => ({
		str: t.text().$type<Brand<string>>(),
		num: t.int4().$type<Brand<number>>(),
		big: t.bigint({ mode: 'bigint' }).$type<Brand<bigint>>(),
		bool: t.bool().$type<Brand<boolean>>(),
		sym: t.text().$type<Brand<symbol>>(),
		date: t.date({ mode: 'date' }).$type<Brand<Date>>(),
		unknown: t.jsonb().$type<Brand<unknown>>(),
		json: t.jsonb().$type<Brand<{ k: 'v' }>>(),
	}));

	const rawtypes = cockroachTable('users', (t) => ({
		str: t.text(),
		num: t.int4(),
		...
		unknown: t.jsonb(),
		json: t.jsonb().$type<{ k: 'v' }>(),
	}));

	const db = drizzle({
		relations: defineRelations({ brandtypes, rawtypes }),
		connection: process.env['COCKROACH_CONNECTION_STRING']!,
	});
