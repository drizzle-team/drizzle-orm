...
test('table - insert', (t) => {
	...
	const expected = z.object({ id: intSchema.optional(), name: textSchema, age: intSchema.optional() });
	expectSchemaShape(t, expected).from(result);
	Expect<Equal<typeof result, typeof expected>>();
});
...
test('nullability - select', (t) => {
	...
	const result = createSelectSchema(table);
	const expected = z.object({
		c1: intSchema.optional(),
		c2: intSchema,
		c3: intSchema.optional(),
		c4: intSchema,
	});
	expectSchemaShape(t, expected).from(result);
	Expect<Equal<typeof result, typeof expected>>();
...
// 	const result = createSelectSchema(view, {
// 		c2: (schema) => schema.max(1000).optional(),
// 		c3: z.string().transform(Number),
// 		nested: {
// 			c5: (schema) => schema.max(1000).optional(),
...
// 		nested: z.object({
// 			c4: intSchema.optional(),
// 			c5: intSchema.max(1000).optional(),
// 			c6: z.string().transform(Number),
// 		}),
// 		table: z.object({
// 			c1: intSchema.optional(),
...
// 			c3: z.string().transform(Number),
// 			c4: intSchema.optional(),
// 			c5: intSchema.optional(),
// 			c6: intSchema.optional(),
// 		}),
// 	});
// 	expectSchemaShape(t, expected).from(result);
// 	type T = Simplify<typeof result>
// 	Expect<Equal<typeof result, typeof expected>>();
// });
...
/* Infinitely recursive type */ {
	...
	const expected = z.object({
		...
		json2: z.optional(TopLevelCondition),
	});
