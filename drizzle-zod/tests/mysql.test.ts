...
test('all data types', (t) => {
	...
	const expected = z.object({
		...
		binary: z.string().max(Number.MAX_SAFE_INTEGER),
		boolean: z.boolean(),
		...
		datetime1: z.date(),
		datetime2: z.string().max(Number.MAX_SAFE_INTEGER),
		decimal1: z.string().max(Number.MAX_SAFE_INTEGER),
		decimal2: z.string().max(Number.MAX_SAFE_INTEGER),
		...
		varbinary: z.string().max(Number.MAX_SAFE_INTEGER),
