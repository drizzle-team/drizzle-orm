import { Named } from "cli/components-api"
import { Patch } from "testrunner"
...
export class PatchResolver {
    ...
    resolveTables<T extends Named>(added: T[], missing: T[]) {
        return applyPatch(added, missing, this.config.tables.sequence)
    }

    resolveColumns<T extends Named>(tableName: string, added: T[], missing: T[]) {
        const resolved = applyPatch(added, missing, this.config.columns_in_tables[tableName].sequence)
