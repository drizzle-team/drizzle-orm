...
export interface JsonCreateReferenceStatement {
...
export type JsonAlterColumnStatement = JsonRenameColumnStatement
...
export type JsonStatement = JsonAlterColumnStatement
...
export const prepareCreateTableJson = (table: Table): JsonCreateTableStatement => {
...
export const prepareCreateEnumJson = (name: string, values: string[]): JsonCreateEnumStatement => {
    return {
        type: 'create_type_enum',
        name: name,
        values
...
export const prepareAddValuesToEnumJson = (name: string, values: string[]): JsonAddValueToEnumStatement[] => {
...
export const prepareRenameColumns = (tableName: string, pairs: { from: Column, to: Column }[]): JsonRenameColumnStatement[] => {
    return pairs.map(it => {
        return {
            type: 'alter_table_rename_column',
            tableName: tableName,
...
const _prepareDropColumns = (taleName: string, columns: Column[]): JsonDropColumnStatement[] => {
...
const _prepareAddColumns = (tableName: string, columns: Column[]): JsonAddColumnStatement[] => {
    return columns.map(it => {
        return {
            type: 'alter_table_add_column',
            tableName: tableName,
...
const _prepareAlterColumns = (tableName: string, columns: AlteredColumn[]): JsonAlterColumnStatement[] => {
    ...
    for (const column of columns) {
        ...
        if (column.notNull?.type === 'changed') {
            const type = column.notNull.new ? 'alter_table_alter_column_set_notnull' : 'alter_table_alter_column_drop_notnull'
            statements.push({
                type: type,
                tableName,
                columnName
...
export const prepareCreateIndexesJson = (tableName: string, indexes: Index[]): JsonCreateIndexStatement[] => {
...
export const prepareCreateReferencesJson = (tableName: string, columns: Column[]): JsonCreateReferenceStatement[] => {
...
export const prepareDropIndexesJson = (tableName: string, indexes: Index[]): JsonDropIndexStatement[] => {
    return indexes.map(index => {
        return {
            type: 'drop_index',
            tableName,
